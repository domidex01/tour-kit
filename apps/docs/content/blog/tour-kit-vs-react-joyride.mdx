...
Yes. The core tour functionality (`@tourkit/core`, `@tourkit/react`, `@tourkit/hints`) is MIT-licensed and free. The Pro license ($9.99, one-time) adds adoption tracking, announcements, checklists, analytics, media embedding, and scheduling. Most teams start with the free packages and upgrade when they need onboarding infrastructure beyond basic tours.
