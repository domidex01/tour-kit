...
description: "Build role-based product tours in React with Tour Kit. Filter steps by admin, editor, or viewer roles using the when prop and React Context."
primary_keyword: "conditional product tour react"
secondary_keywords: ["role-based onboarding react", "user segmentation product tour", "conditional tour steps react", "react product tour user role"]
category: "Tutorials"
...
Tour Kit ships as two packages for this tutorial: `@tourkit/core` (under 23KB gzipped) for the tour engine and step evaluation logic, and `@tourkit/react` for the React component bindings that render steps in your component tree.
...
<table>
  ...
  <tbody>
    <tr>
      <td>Admin</td>
      <td>6</td>
      <td>billing, team-management</td>
      <td>welcome, content-editor, media-library, help-center</td>
    </tr>
    <tr>
      <td>Editor</td>
      <td>4</td>
    ...
    <tr>
      <td>Viewer</td>
      <td>3 (separate tour)</td>
      <td>viewer-welcome, viewer-reports, viewer-export</td>
      <td>(uses standalone tour)</td>
    </tr>
  </tbody>
</table>
...
- Adding Tour Kit's [@tourkit/analytics](/docs/analytics) package to track completion rates per role. A 90% completion rate for admins but 30% for editors tells you the editor tour needs work.
- Using [@tourkit/scheduling](/docs/scheduling) to delay tours until a user has had 24 hours to explore on their own. As of April 2026, 74% of users prefer adaptive onboarding that lets them skip steps they've already figured out ([UserGuiding, 2026](https://userguiding.com/blog/user-onboarding-statistics)).
...
Yes. Tour Kit supports React Server Components by keeping all tour logic in client components. Wrap your `TourProvider` in a `'use client'` file, pass the user role from a server component via props, and the `when` filtering works the same way. See the [Next.js App Router tutorial](/blog/nextjs-app-router-product-tour) for the full setup.

### Does adding a conditional product tour affect performance?

Tour Kit's core is under 23KB gzipped. The `when` callbacks are plain synchronous functions with no DOM queries or network calls. We measured initialization at under 2ms with 20 conditional steps on Vite + React 19. Spotlight overlays use GPU-accelerated CSS transforms, not DOM re-parenting.
...
<script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
  ...
  "@type": "TechArticle",
  "headline": "Building conditional product tours based on user role",
  "description": "Build role-based product tours in React with Tour Kit. Filter steps by admin, editor, or viewer roles using the when prop and React Context.",
  "author": {
    "@type": "Organization",
    "name": "Tour Kit",
  ...
  "image": "https://tourkit.dev/og-images/conditional-product-tour-user-role.png",
  "url": "https://tourkit.dev/blog/conditional-product-tour-user-role",
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://tourkit.dev/blog/conditional-product-tour-user-role"
  },
  "keywords": ["conditional product tour react", "role-based onboarding react", "user segmentation product tour"],
...
<script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
  ...
  "itemListElement": [
    ...
    { "@type": "ListItem", "position": 3, "name": "Building conditional product tours based on user role", "item": "https://tourkit.dev/blog/conditional-product-tour-user-role" }
  ]
}) }} />
...
<script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "How does Tour Kit filter steps by user role?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit uses the when prop on each step definition. The when function receives the full tour context, including any custom data set via setData(). You store the user role with setData('userRole', role), then each step's when callback checks if the current role matches. Steps where when returns false are skipped entirely."
      }
    },
    {
      "@type": "Question",
      "name": "Can I use this pattern with Next.js App Router?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Yes. Tour Kit supports React Server Components by keeping all tour logic in client components. Wrap your TourProvider in a 'use client' file, pass the user role from a server component via props, and the when filtering works the same way."
      }
    },
    {
      "@type": "Question",
      "name": "Does adding a conditional product tour affect performance?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit's core bundle is under 23KB gzipped. The when callbacks are plain synchronous functions with no DOM queries or network calls. Initialization takes under 2ms in a Vite + React 19 project with 20 conditional steps."
      }
    },
    {
      "@type": "Question",
      "name": "What happens if a user has multiple roles?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "The forRoles helper accepts multiple roles. If your auth system returns an array of roles, modify the guard to check for intersection: roles.some(r => userRoles.includes(r)). Tour Kit doesn't impose a role model. You control the logic inside when."
      }
    },
    {
      "@type": "Question",
      "name": "Is client-side role filtering secure?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "No. Client-side role checks improve UX but don't enforce authorization. A user can modify localStorage or React state to change their apparent role. Always validate permissions on the server. The tour is cosmetic; your API is the security boundary."
      }
    }
  ]
}) }} />
