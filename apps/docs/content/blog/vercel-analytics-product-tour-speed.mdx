...
description: "Connect Tour Kit to Vercel Analytics and Speed Insights to measure how product tours affect Core Web Vitals. TypeScript code, CLS/INP gotchas included."
primary_keyword: "vercel analytics product tour"
secondary_keywords: ["vercel speed insights onboarding", "performance impact tour", "core web vitals product tour"]
...
If you need a tour first, the [Next.js App Router tutorial](/blog/nextjs-app-router-product-tour) covers setup from scratch.
  ...
  return (
    <html lang="en">
      <body>
        ...
        <SpeedInsights
          beforeSend={(data) => {
            // Tag speed data with tour presence
            // Check if a tour overlay exists in the DOM
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>≤ 2.5s</td>
      <td>Medium</td>
      <td>Large tour images becoming the LCP element</td>
    </tr>
    <tr>
      <td>CLS</td>
      <td>≤ 0.1</td>
      <td>High</td>
      <td>Overlay rendering that shifts existing content</td>
    </tr>
    <tr>
      <td>INP</td>
      <td>≤ 200ms</td>
      <td>Medium</td>
      <td>Heavy JS on "Next" button clicks (spotlight repositioning)</td>
    </tr>
    <tr>
      <td>FCP</td>
      <td>≤ 1.8s</td>
      <td>Low</td>
      <td>Tour JS blocking first paint (lazy-load to avoid)</td>
    </tr>
    <tr>
      <td>TTFB</td>
      <td>&lt; 800ms</td>
      <td>None</td>
      <td>Server-side, tours don't affect this</td>
    </tr>
  </tbody>
</table>
...
Tour Kit renders overlays via React portals, outside the main document flow. The tooltip and backdrop don't push existing content around when they appear. Libraries that inject elements inline cause surrounding content to reflow, directly contributing to CLS. Core ships at under 23KB gzipped.

### Will adding two Vercel packages hurt my bundle size?

Both `@vercel/analytics` and `@vercel/speed-insights` load as small async scripts. They don't block rendering or contribute to your main JS bundle in a meaningful way. Combined with Tour Kit's core (under 23KB gzipped), the total footprint of the tour + analytics integration stays well under 20KB. For comparison, a single SaaS onboarding tool's injected script typically runs 80-200KB.
...
- Link from [Core Web Vitals article](/blog/onboarding-tool-core-web-vitals) to this article (Vercel-specific monitoring)
- Link from [GA4 integration article](/blog/ga4-tour-kit-event-tracking-onboarding) (cross-reference alternative analytics)
- Link from [Lighthouse performance article](/blog/onboarding-tool-lighthouse-performance) (lab vs RUM comparison)
- Link to [Tour Kit docs](https://usertourkit.com/) from the provider setup section

**Distribution checklist:**
- Dev.to (canonical back to tourkit.dev)
- Hashnode (canonical back to tourkit.dev)
- Reddit r/nextjs (Vercel + Next.js angle)
- Reddit r/reactjs (performance monitoring angle)

<script type="application/ld+json">
{JSON.stringify({
  "@context": "https://schema.org",
  "@type": "TechArticle",
  "headline": "Tour Kit + Vercel Analytics: measuring tour impact on speed",
  "description": "Connect Tour Kit to Vercel Analytics and Speed Insights to measure how product tours affect Core Web Vitals. TypeScript code, CLS/INP gotchas included.",
  "author": {
    "@type": "Person",
    "name": "Dominik Dex",
  ...
  "image": "https://usertourkit.com/og-images/vercel-analytics-product-tour-speed.png",
  "url": "https://usertourkit.com/blog/vercel-analytics-product-tour-speed",
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://usertourkit.com/blog/vercel-analytics-product-tour-speed"
  },
  "keywords": ["vercel analytics product tour", "vercel speed insights onboarding", "performance impact tour", "core web vitals product tour"],
  "proficiencyLevel": "Intermediate",
  "dependencies": "React 18+, TypeScript 5+, Vercel Pro plan",
  "programmingLanguage": {
...
{JSON.stringify({
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "Does Vercel Analytics track events in development mode?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Vercel Analytics does not fire events in development mode. Both @vercel/analytics and @vercel/speed-insights only collect data on deployed Vercel previews and production. To verify your track() calls work, deploy to a preview branch and check the Vercel dashboard within a few minutes."
      }
    },
    {
      "@type": "Question",
      "name": "Can I use Vercel Analytics custom events on the free plan?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Custom events through the track() function require a Vercel Pro or Enterprise plan. The free tier includes pageview analytics and Speed Insights basic metrics (all Core Web Vitals), but not custom event tracking."
      }
    },
    {
      "@type": "Question",
      "name": "How does Tour Kit minimize CLS compared to other tour libraries?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit renders overlay elements via React portals, placing them outside the main document flow. This means the tour tooltip and backdrop don't push existing page content around when they appear. Tour Kit's core ships at under 23KB gzipped with zero runtime dependencies."
      }
    },
    {
      "@type": "Question",
      "name": "Will adding two Vercel packages hurt my bundle size?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Both @vercel/analytics and @vercel/speed-insights load as small async scripts. They don't block rendering or contribute to your main JS bundle in a meaningful way. Combined with Tour Kit's core (under 23KB gzipped), the total footprint stays well under 20KB."
      }
    },
    {
      "@type": "Question",
      "name": "Can I track tour events server-side?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Vercel Analytics supports server-side tracking via import { track } from '@vercel/analytics/server'. This is useful for logging tour completion events in API routes. Server-side events bypass ad blockers entirely."
      }
    }
  ]
})}
