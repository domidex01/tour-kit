...
description: "Why I chose MIT over AGPL and BSL for Tour Kit — a decision since reversed, with Tour Kit now published under BSL 1.1. Real data from HashiCorp, Redis, Elastic, and Shepherd.js shaped the original license choice."
...
> **Update, 15 September 2026 — Tour Kit is no longer MIT, and this post argues
> against the licence it now carries.**
> On 14 September 2026 `@tour-kit/core`, `react`, `hints`, `vue` and `svelte`
> published as **BSL 1.1** in 3.0.0: free for development, evaluation, testing and
> CI, a one-time key to serve it to end users, and each published version
> converting to MIT on its Change Date (2030-09-14). That is the BSL this post
> spends two hundred lines warning about. I am leaving the argument up unedited,
> because it was made in good faith and the risks it names are real ones I have
> now chosen to take on. What it did not price in was a year of shipping: roughly
> 8,000 installs a month and no revenue to pay for the maintenance the post
> promises. See [pricing](/pricing) for the current model.
...
As of April 2026, the library has 10 packages, ships under 23KB gzipped for the core, and works with React 18 and 19. Three packages are MIT. Eight are paid. The split works because the boundary is honest.
