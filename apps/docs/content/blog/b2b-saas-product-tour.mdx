...
Tour Kit's approach: install only the packages you need. If you just want tours, that's `@tourkit/core` + `@tourkit/react` at under 23KB. Add `@tourkit/analytics` when you're ready to track, `@tourkit/checklists` when you need progress tracking. Each package tree-shakes independently.
