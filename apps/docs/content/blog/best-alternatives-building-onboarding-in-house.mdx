...
Tour Kit is a headless onboarding library for React that ships logic without prescribing UI. The core package weighs under 23KB gzipped with zero runtime dependencies. You render steps with your own components (Radix primitives, shadcn/ui, Tailwind classes, whatever your team already uses). WCAG 2.1 AA compliance is built in: focus trapping, ARIA roles, keyboard navigation, and `prefers-reduced-motion` support all ship by default.
...
- MIT license, $9.99 one-time for Pro features (no per-seat, no MAU pricing)
...
Free (MIT) for core packages. Pro features (adoption tracking, surveys, scheduling) cost $9.99 one-time per project.
...
- 37KB gzipped is 1.6x larger than Tour Kit's core, which is noticeable on mobile connections
...
React Joyride, Shepherd.js, Driver.js, and Onborda are MIT-licensed and free. Tour Kit's core is BUSL-1.1-licensed source-available (free in development, licence key in production). Tour Kit provides a headless architecture under 23KB gzipped with WCAG 2.1 AA compliance. React Joyride has the largest community at 340K+ weekly downloads. Your choice depends on framework and design system requirements.
