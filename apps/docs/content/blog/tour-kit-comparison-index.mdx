...
Tour Kit's pricing model is simple. The core three packages (`@tourkit/core`, `@tourkit/react`, `@tourkit/hints`) are MIT-licensed and free forever. The seven extended packages require a one-time [$99 Pro license](/pricing). No monthly fees. No per-MAU pricing.
