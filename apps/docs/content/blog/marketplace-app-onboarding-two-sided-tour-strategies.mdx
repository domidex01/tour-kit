...
**Tour Kit** handles role-based routing natively through its provider architecture. The headless design means you style tooltips and checklists with your existing design system, not fighting CSS overrides from a third-party brand. The `@tour-kit/checklists` and `@tour-kit/analytics` packages solve the two patterns this use case needs most. Tour Kit ships at under 23KB gzipped for core + React. Limitation worth noting: Tour Kit requires React 18+ and doesn't have a visual builder, so your team needs developers comfortable with JSX. [Get started at usertourkit.com](https://usertourkit.com/).
...
Tour Kit's core and React packages are BUSL-1.1-licensed and source-available. For marketplace onboarding, the `@tour-kit/checklists` and `@tour-kit/analytics` packages require the Pro license at $9.99 one-time. No subscriptions, no per-MAU pricing that scales against you as your marketplace grows.
