...
Tour Kit's core and React packages are MIT-licensed and free. For marketplace onboarding, the `@tour-kit/checklists` and `@tour-kit/analytics` packages require the Pro license at $9.99 one-time. No subscriptions, no per-MAU pricing that scales against you as your marketplace grows.
