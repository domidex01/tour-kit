...
I wanted a headless library where the logic lives in hooks and the UI is yours. Tree-shakes to under 23KB for the core. TypeScript strict mode. WCAG 2.1 AA from the start, not patched in later.
...
**Bundle size as a positioning wedge.** Tour Kit's core ships under 23KB gzipped. In a market where most alternatives are 20-40KB, that number does real work in blog posts and comparison tables. Developers who care about Core Web Vitals noticed.
...
The Pro tier pricing page. I launched with a $9.99 one-time license for the extended packages. At month 1, with 183 stars and no proven community, a pricing page adds friction without adding revenue.
