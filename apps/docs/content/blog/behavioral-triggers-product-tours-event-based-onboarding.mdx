...
description: "Build event-based product tours that trigger on user actions, not timers. Code examples for click, route, inactivity, and compound triggers in React."
primary_keyword: "behavioral trigger product tour"
secondary_keywords: ["event-based onboarding", "trigger tour on user action", "product tour trigger conditions", "behavioral targeting onboarding"]
category: "Metrics"
...
<table>
  ...
  <tbody>
    <tr>
      <td>On-page position (contextual)</td>
      <td>69.56%</td>
      <td>Chameleon, 15M interactions</td>
    </tr>
    <tr>
      <td>Click-triggered (user-initiated)</td>
      <td>67%</td>
      <td>Chameleon, 15M interactions</td>
    </tr>
    <tr>
      <td>Launcher-triggered</td>
      <td>61.65%</td>
      <td>Chameleon, 15M interactions</td>
    </tr>
    <tr>
      ...
      <td>Chameleon benchmarks</td>
    </tr>
    <tr>
      <td>Smart Delay (inactivity)</td>
      <td>+21% above fixed delay</td>
      <td>Chameleon benchmarks</td>
    </tr>
    <tr>
      <td>Time/delay-triggered (fixed)</td>
      <td>31%</td>
      <td>Chameleon, 15M interactions</td>
    </tr>
  </tbody>
...
Tour Kit's `@tourkit/analytics` package fires callbacks for `tour:started`, `tour:completed`, `tour:dismissed`, and `step:viewed`. Pipe these into PostHog, Amplitude, or Mixpanel to build trigger-specific funnels. We covered the exact wiring in our [PostHog event tracking guide](https://usertourkit.com/blog/track-product-tour-completion-posthog-events) and [Amplitude retention guide](https://usertourkit.com/blog/amplitude-tour-kit-onboarding-retention).
...
Behavioral triggers can fire more often than timers because there are more conditions to match. If a user hits 4 trigger conditions in one session, showing 4 tours is worse than showing none. Cap triggered tours at 1-2 per session. Tour Kit's `@tour-kit/surveys` package includes fatigue prevention logic that applies to any triggered content, not just surveys.

### No persistence across sessions

A user dismisses the tour on Tuesday. The same behavioral trigger fires on Wednesday. Without session-aware storage, dismissed tours come back like uninvited guests. Use `localStorage`, a database flag, or Tour Kit's storage adapters to track which tours a user has seen or dismissed. We wrote a full guide on [tour progress persistence with localStorage](https://usertourkit.com/blog/tour-progress-persistence-localstorage).
...
```json
{
  ...
  "@type": "TechArticle",
  "headline": "Behavioral triggers for product tours: event-based onboarding",
  "description": "Build event-based product tours that trigger on user actions, not timers. Code examples for click, route, inactivity, and compound triggers in React.",
  "author": {
    "@type": "Person",
    "name": "Dominic Bérubé",
  ...
  "image": "https://usertourkit.com/og-images/behavioral-triggers-product-tours-event-based-onboarding.png",
  "url": "https://usertourkit.com/blog/behavioral-triggers-product-tours-event-based-onboarding",
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://usertourkit.com/blog/behavioral-triggers-product-tours-event-based-onboarding"
  },
  "keywords": ["behavioral trigger product tour", "event-based onboarding", "trigger tour on user action", "product tour trigger conditions"],
...
- Link FROM: `track-product-tour-completion-posthog-events`, `amplitude-tour-kit-onboarding-retention`, `aha-moment-framework-tours-activation-events`, `conditional-product-tour-user-role`, `state-machine-pattern-complex-tour-flows`
- Link TO: `tour-progress-persistence-localstorage`, `track-product-tour-completion-posthog-events`, `amplitude-tour-kit-onboarding-retention`

**Distribution checklist:**
- Dev.to (with canonical URL to usertourkit.com)
- Hashnode (with canonical URL)
- Reddit r/reactjs — "How we increased tour completion 2x by switching from timers to behavioral triggers"
