...
Tour Kit's core is under 23KB gzipped with zero runtime dependencies. In our test suite, adding tour component tests added roughly 200ms to the total run time on Vitest's jsdom environment. The overhead comes from rendering the provider tree, not from the library itself.
  ...
  },
  {
    ...
    "mainEntity": [
      ...
      },
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Tour Kit's core is under 23KB gzipped with zero runtime dependencies. In our test suite, adding tour component tests added roughly 200ms to the total run time on Vitest's jsdom environment. The overhead comes from rendering the provider tree, not from the library itself."
        }
      }
    ]
  }
