...
Tour Kit's core bundle is under 23KB gzipped, so the performance impact on test execution is negligible. The bigger factor is animation timing: each step transition adds 300ms of wait time by default. For a 7-step tour, that's about 2 extra seconds. Disable animations in test mode with `animated={false}` to cut that to near-zero.
...
<script type="application/ld+json">{`
{
  ...
  "mainEntity": [
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit's core is under 23KB gzipped, so the performance impact on test execution is negligible. Animation timing is the bigger factor — each step transition adds 300ms. Disable animations in test mode with animated={false} to cut that to near-zero."
      }
    },
    {
      "@type": "Question",
