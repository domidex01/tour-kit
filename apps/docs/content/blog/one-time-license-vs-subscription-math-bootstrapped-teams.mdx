...
We built Tour Kit with this in mind. The core packages are BUSL-1.1-licensed and source-available. Tour Kit Pro is a one-time purchase: you get the code, it runs in your app, and there's no server calling home. The breakeven versus a $89/month subscription is about three weeks.
...
We calculated our own tool budget when building Tour Kit. At $89-249/month for onboarding alone, it would've consumed 15-50% of our total dev tool spend. That wasn't tenable. So we built the tool we wanted: free to develop against, one-time to ship, no monthly drain.
...
It depends on the vendor's model. A Tour Kit licence includes the version you purchase plus all patch and minor updates. Major version upgrades are separate purchases at a discount. Each published version also converts to MIT on its Change Date. Your existing code keeps working regardless because there's no server dependency that could be turned off.
