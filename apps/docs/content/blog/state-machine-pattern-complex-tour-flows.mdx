...
XState v5 tree-shakes well. If you use `setup()` and `createMachine()` without the interpreter, the core logic is around 7KB gzipped. Adding `@xstate/react` brings the total to approximately 10KB. Combined with Tour Kit's core (under 23KB) and React bindings (under 12KB), the complete stack is under 30KB gzipped for a fully state-machine-driven tour system.
