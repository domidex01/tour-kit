...
description: "Migrate from Shepherd.js to Tour Kit step by step. Replace AGPL licensing with MIT, get native React hooks, and cut your tour bundle to under 23KB gzipped."
primary_keyword: "migrate shepherd js tour kit"
secondary_keywords: ["shepherd js migration", "replace shepherd js react", "shepherd js alternative mit", "shepherd js agpl license"]
category: "Tutorials"
...
Tour Kit's entire codebase (all 10 packages) is public. The core, React, hints, Vue and Svelte packages are BSL 1.1 and convert to MIT on each release's Change Date; a [licence](/pricing) at $9.99 one-time covers serving any of them to end users.
...
Shepherd.js and Tour Kit share the same core concepts (steps, targets, callbacks, overlays) but differ in how they're expressed. Shepherd uses class instances and imperative methods while Tour Kit uses React context, hooks, and declarative props. The table below maps every Shepherd concept to its Tour Kit equivalent so you can plan your conversion before writing code.
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>Shepherd.Tour</code></td>
      <td><code>TourProvider</code></td>
      <td>React context instead of class instance</td>
    </tr>
    <tr>
      <td><code>tour.addStep()</code></td>
      <td><code>steps</code> prop on provider</td>
      <td>Declarative array instead of imperative calls</td>
    </tr>
    <tr>
      <td><code>tour.start()</code></td>
      <td><code>useTour().start()</code></td>
      <td>Hook-based, reactive state</td>
    </tr>
    <tr>
      <td><code>attachTo: &#123; element &#125;</code></td>
      <td><code>target</code> selector or ref</td>
      <td>Same concept, different syntax</td>
    </tr>
    <tr>
      <td><code>when: &#123; show, hide &#125;</code></td>
      <td><code>onStepChange</code> callback</td>
      <td>Single callback vs per-event handlers</td>
    </tr>
    <tr>
      <td>Shepherd CSS import</td>
      <td>Your own components</td>
      <td>Zero shipped CSS, you render the tooltip</td>
    </tr>
    <tr>
      <td><code>shepherd.js</code> (AGPL-3.0)</td>
      <td><code>@tourkit/core</code> (BSL 1.1)</td>
      <td>No license risk</td>
    </tr>
  </tbody>
</table>
...
The safest migration strategy is running both libraries simultaneously, converting one tour at a time, and removing Shepherd only after all tours pass QA. Tour Kit's core adds under 23KB gzipped with zero runtime dependencies, so the temporary overlap barely affects your bundle. Install it alongside Shepherd:
...
Converting a Shepherd tour to Tour Kit means replacing imperative `tour.addStep()` calls with a declarative steps array, swapping class-based event handlers for React hooks, and writing your own tooltip JSX instead of relying on Shepherd's built-in UI. Here's a typical Shepherd.js tour and its Tour Kit equivalent.
...
After every tour is converted and passing QA, you can safely remove Shepherd.js from your project. This is the step that actually eliminates the AGPL dependency from your `node_modules` and cuts your bundle size. Run the uninstall:
...
Your bundle drops by 13.7KB gzipped (Shepherd) and gains under 23KB gzipped (Tour Kit core). Net saving: roughly 6KB gzipped. More importantly, you've removed the AGPL dependency entirely.

## Common issues and troubleshooting

These are the four problems developers hit most often when migrating from Shepherd.js to Tour Kit, based on the architectural differences between the two libraries. Each includes the root cause and the fix.
...
Tour Kit doesn't ship a modal overlay because it's headless. You build it yourself, which sounds like more work but means you control the z-index stacking, animation, and backdrop opacity. A basic overlay is roughly 20 lines of CSS. The [Tour Kit overlay guide](/docs/guides/overlays) shows how to match Shepherd's overlay behavior with your own styles.
...
Once you've migrated tours, you can add `@tourkit/analytics` for drop-off funnels and completion rates, `@tourkit/checklists` for onboarding task lists with dependencies, `@tourkit/announcements` for modals and banners, `@tourkit/surveys` for NPS/CSAT/CES microsurveys, `@tourkit/hints` for persistent beacons, and `@tourkit/adoption` for feature adoption tracking.
...
Once the migration is complete, you can start using Tour Kit features that Shepherd.js doesn't offer. Here's a practical sequence for the first two weeks after migration.

1. Run both libraries side by side for a sprint to validate the migration
2. Convert your simplest tour first to build confidence
3. Add `@tourkit/analytics` to track whether the migrated tours perform differently
4. Delete the Shepherd dependency once all tours pass QA
5. Explore the [Tour Kit documentation](/docs) for patterns Shepherd didn't support
...
<script type="application/ld+json">{JSON.stringify({
  ...
  "@type": "TechArticle",
  "headline": "Migrating from Shepherd.js to Tour Kit: AGPL to MIT",
  "description": "Migrate from Shepherd.js to Tour Kit step by step. Replace AGPL licensing with MIT, get native React hooks, and cut your tour bundle to under 23KB gzipped.",
  "author": {
    "@type": "Person",
  ...
  "image": "https://tourkit.dev/og-images/migrate-shepherd-js-tour-kit.png",
  "url": "https://tourkit.dev/blog/migrate-shepherd-js-tour-kit",
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://tourkit.dev/blog/migrate-shepherd-js-tour-kit"
  },
  "keywords": ["migrate shepherd js tour kit", "shepherd js migration", "replace shepherd js react", "shepherd js alternative mit"],
...
<script type="application/ld+json">{JSON.stringify({
  ...
  "itemListElement": [
    ...
    },
    {
      "@type": "ListItem",
      "position": 3,
      "name": "Migrating from Shepherd.js to Tour Kit: AGPL to MIT",
      "item": "https://tourkit.dev/blog/migrate-shepherd-js-tour-kit"
    }
  ]
})}</script>
...
<script type="application/ld+json">{JSON.stringify({
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "Does migrating from Shepherd.js to Tour Kit require rewriting all tour content?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit uses a different step format than Shepherd.js, so you'll rewrite step definitions, but the content itself (titles, body text, target selectors) transfers directly. A 10-step tour typically takes 15-20 minutes to convert. The bigger change is building your own tooltip component, which Tour Kit requires because it's headless."
      }
    },
    {
      "@type": "Question",
      "name": "Is Tour Kit free for commercial use?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit's core packages (@tourkit/core, @tourkit/react, @tourkit/hints) are MIT-licensed with no restrictions on commercial use, distribution, or modification. Unlike Shepherd.js's AGPL-3.0, MIT doesn't require source code disclosure for network-accessible applications. Tour Kit Pro ($9.99 one-time) adds extended packages but the core tour engine remains MIT."
      }
    },
    {
      "@type": "Question",
      "name": "Can I use Tour Kit with the same CSS I wrote for Shepherd.js?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "No. Shepherd.js ships its own DOM structure and CSS classes. Tour Kit doesn't render any DOM, so you bring your own components. Your Tailwind classes and design tokens work without specificity fights."
      }
    },
    {
      "@type": "Question",
      "name": "How does Tour Kit handle accessibility compared to Shepherd.js?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit is built for WCAG 2.1 AA compliance with keyboard navigation, focus management, screen reader announcements, and prefers-reduced-motion support built into the core. Shepherd.js provides basic accessibility but doesn't target a specific WCAG level. We tested Tour Kit against axe-core and Lighthouse, scoring 100 on accessibility audits."
      }
    },
    {
      "@type": "Question",
      "name": "What happens to my Shepherd.js analytics after migration?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Shepherd.js doesn't include built-in analytics. If you wired up custom event tracking, map those same events to Tour Kit's onStepChange, onComplete, and onDismiss callbacks. For structured analytics, @tourkit/analytics provides plugin-based integrations with PostHog, Mixpanel, Amplitude, and GA4."
      }
    }
  ]
})}</script>
