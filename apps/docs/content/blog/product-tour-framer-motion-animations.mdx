...
Motion (formerly Framer Motion) fits product tours best thanks to `AnimatePresence` for step transitions and `layoutId` for morphing spotlights. Built-in `prefers-reduced-motion` support means zero extra accessibility code. Tour Kit pairs with Motion out of the box at under 23KB initial cost. That said, Tour Kit is our project, so weigh this accordingly.
