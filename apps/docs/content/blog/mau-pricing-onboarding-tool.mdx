...
Tour Kit's core ships at under 23KB gzipped with zero runtime dependencies. You can add analytics ([`@tour-kit/analytics`](/docs/analytics/overview)), checklists ([`@tour-kit/checklists`](/docs/checklists/overview)), or surveys ([`@tour-kit/surveys`](/docs/surveys/overview)). Each is a separate package, installed only when needed.
...
[Try Tour Kit](https://tourkit.dev/docs/getting-started): zero MAU fees, free while you build. Check the [live demo on StackBlitz](https://tourkit.dev/docs/examples) to see what headless onboarding looks like in practice.
...
Open-source libraries like Tour Kit replace the tour and onboarding functionality (step-by-step guides, tooltips, checklists, announcements) without MAU-based billing. Tour Kit's core is under 23KB gzipped with zero dependencies. You need React developers and won't get a visual builder. For engineering teams already building in React, the implementation cost of 1 to 3 days is typically recovered within the first month compared to SaaS pricing.
