...
Tour Kit's pricing doesn't scale with your user count. It is free while you build and $9.99 one-time to ship. We don't charge per MAU because we think that model penalizes growth. (Bias disclosure: we built Tour Kit, so factor that into your evaluation.)
