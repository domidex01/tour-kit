...
User Tour Kit, Onborda, NextStepjs, and Driver.js all work with Next.js App Router when placed inside a `"use client"` boundary. Tour Kit is the only headless option with built-in App Router navigation adapters, typed step definitions, and WCAG 2.1 AA accessibility — at under 23 KB gzipped for the core package. React Joyride and Shepherd.js work too, but both need extra configuration to avoid SSR errors and neither supports multi-route tours natively.
...
- **Tour Kit core + react:** Added ~23 KB to the client bundle (gzipped). Tree-shakes unused features.
...
Driver.js ships at roughly 5 KB gzipped but has no React bindings or multi-route support. Tour Kit's core is under 23 KB gzipped with TypeScript types, accessibility, and App Router navigation adapters. React Joyride is the largest at approximately 85 KB gzipped.
