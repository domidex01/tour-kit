...
The catch: OTel's browser SDK adds roughly 30-40KB gzipped depending on which instrumentations you load ([Bundlephobia](https://bundlephobia.com/package/@opentelemetry/sdk-trace-web)). On top of Tour Kit's core at under 23KB gzipped, that's meaningful. For enterprise apps already running OTel for backend tracing, the frontend SDK is a natural extension. For a marketing site with a 3-step tour? Overkill. Use PostHog or GA4 instead.
...
The `@opentelemetry/sdk-trace-web` package with OTLP HTTP exporter adds roughly 30-40KB gzipped. Combined with Tour Kit's core at under 23KB gzipped, total is about 53-63KB. If your app already loads OTel for fetch tracing, the incremental cost of this plugin is zero since it reuses the existing provider.
...
<script type="application/ld+json">{JSON.stringify({
  ...
  "mainEntity": [
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "The @opentelemetry/sdk-trace-web package with the OTLP HTTP exporter adds roughly 30-40KB gzipped. Combined with Tour Kit's core at under 23KB gzipped, total overhead is about 53-63KB. For apps already loading OTel, the incremental cost of the Tour Kit plugin is zero."
      }
    },
    {
      "@type": "Question",
