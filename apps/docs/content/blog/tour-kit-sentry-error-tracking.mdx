...
Yes. The lifecycle callbacks (`onTourStart`, `onStepView`, `onTourComplete`, `onTourSkip`) are part of `@tourkit/react`, which is BUSL-1.1-licensed and source-available. The `@tourkit/analytics` plugin route is also free. Nothing in this integration requires Tour Kit Pro.

### How much does Sentry add to my bundle?

As of April 2026, `@sentry/react` v7 is 52.67KB minified (down 29% from v6). With tree-shaking, importing only `init`, `addBreadcrumb`, `setTag`, and `ErrorBoundary` brings the gzipped impact under 20KB. As Shu Ding from Vercel noted, "We were able to reduce [the bundle] further through tree shaking" ([Sentry Blog](https://blog.sentry.io/javascript-sdk-package-reduced/)). Sentry also supports lazy loading, which defers the cost until after initial page load. Combined with Tour Kit core (under 23KB gzipped), the entire monitored tour system adds under 28KB.
