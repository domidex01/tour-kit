...
description: "Wire Tour Kit's onComplete callback to HubSpot's Contacts API. Update lifecycle stages and automate CRM follow-ups with TypeScript code."
primary_keyword: "hubspot onboarding tour integration"
secondary_keywords: ["hubspot product tour trigger", "crm onboarding integration", "hubspot workflow automation react"]
...
We built this integration for a B2B SaaS dashboard and hit three gotchas along the way. Every code sample below runs against HubSpot's free CRM tier and Tour Kit's BUSL-1.1 source-available packages. No $890/month Marketing Hub Professional required.
...
<table>
<thead>
<tr><th>Step</th><th>System</th><th>Action</th></tr>
</thead>
<tbody>
<tr><td>1</td><td>Tour Kit (browser)</td><td><code>onComplete</code> fires with tour ID and step count</td></tr>
<tr><td>2</td><td>Your API route (server)</td><td>POST <code>/api/tour-completed</code> with user ID and tour metadata</td></tr>
<tr><td>3</td><td>HubSpot Contacts API</td><td>PATCH <code>/crm/v3/objects/contacts/&#123;id&#125;</code>, set <code>product_tour_completed = true</code></td></tr>
<tr><td>4</td><td>HubSpot Workflow</td><td>Enrolls on property change, updates lifecycle stage, sends email</td></tr>
</tbody>
</table>
...
<table>
<thead>
<tr><th>Tour event</th><th>HubSpot lifecycle stage</th><th>Workflow action</th></tr>
</thead>
<tbody>
<tr><td>Signed up, no tour started</td><td>Subscriber</td><td>Send welcome email</td></tr>
<tr><td>Completed onboarding tour</td><td>Lead</td><td>Assign to sales rep</td></tr>
<tr><td>Used key feature post-tour</td><td>MQL</td><td>Create deal, notify AE</td></tr>
<tr><td>Clicked upgrade CTA in tour</td><td>SQL</td><td>Schedule demo, priority routing</td></tr>
</tbody>
</table>
...
Chameleon syncs HubSpot Lists into its targeting engine and pushes property updates back ([Chameleon docs](https://www.chameleon.io/integrations/hubspot)). It's managed and no-code. Tour Kit is code-first: you own the API route, data shape, and deployment. Chameleon charges per MAU; Tour Kit is $9.99 one-time Pro or free BUSL-1.1 in development.

{/* JSON-LD Schema */}
export const jsonLd = {
  "@context": "https://schema.org",
  "@type": "TechArticle",
  "headline": "Tour Kit + HubSpot: trigger CRM workflows from tour completion",
  "description": "Wire Tour Kit's onComplete callback to HubSpot's Contacts API and workflow triggers. Update lifecycle stages, enroll contacts, and automate follow-ups with TypeScript.",
  "author": {
    "@type": "Person",
  ...
  "image": "https://usertourkit.com/og-images/tour-kit-hubspot-crm-workflows.png",
  "url": "https://usertourkit.com/blog/tour-kit-hubspot-crm-workflows",
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://usertourkit.com/blog/tour-kit-hubspot-crm-workflows"
  },
  "keywords": ["hubspot onboarding tour integration", "hubspot product tour trigger", "crm onboarding integration"],
  "proficiencyLevel": "Intermediate",
  "dependencies": "React 18+, TypeScript 5+, HubSpot CRM (free tier)",
  "programmingLanguage": {
...
- Link FROM: /blog/track-product-tour-completion-posthog-events (similar analytics integration pattern)
- Link FROM: /blog/amplitude-tour-kit-onboarding-retention (analytics integration cluster)
- Link FROM: /blog/ga4-tour-kit-event-tracking-onboarding (GA4 integration cluster)
- Link FROM: /blog/best-product-tour-tools-b2b-saas (mention HubSpot integration as differentiator)
- Link TO: /blog/composable-tour-library-architecture (explains the plugin system)
- Link TO: /blog/building-plugin-system-product-tour-library (deep-dive on plugin interface)

Distribution checklist:
- Dev.to (canonical to usertourkit.com/blog/tour-kit-hubspot-crm-workflows)
- Hashnode (canonical URL)
- Reddit r/reactjs — "How we wired product tour events to HubSpot CRM workflows"
- Reddit r/hubspot — "Code-first product tours that update HubSpot contacts automatically"
