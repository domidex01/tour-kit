...
**Tour Kit** is a 10-package React monorepo. Core logic, hints, checklists, analytics, announcements, surveys, and scheduling are separate packages. Install only what you need. BSL 1.1: free while you build, a one-time key from $9.99 to ship. We built Tour Kit, so take this with appropriate skepticism.
