...
Tour Kit's core ships at under 23KB gzipped with zero runtime dependencies. The portal rendering decision is yours, not the library's. That said, Tour Kit requires React 18+ and doesn't have a visual tour builder, so you need developers who are comfortable writing React components.
