...
We'll replace an Intro.js tour with Tour Kit, a headless React library that ships under 23KB gzipped (core) and uses BUSL-1.1 licensing (free in development). Every code example below compiles against `@tourkit/core` 0.x and `@tourkit/react` 0.x.
...
Tour Kit addresses all five. It renders nothing by default (you provide the UI), runs inside React's component tree, uses BUSL-1.1 licensing, and ships with WCAG 2.1 AA compliance including focus trapping and full ARIA attributes.
...
Completing this migration removes the AGPL-3.0 license from your dependency tree, cuts roughly 8.5KB from your gzipped JavaScript bundle (16.5KB Intro.js down to under 23KB Tour Kit core), and replaces a CSS-override workflow with tooltip components that use your own design system natively. You also pick up focus trapping, full ARIA attributes, and built-in persistence that Intro.js required manual code to achieve.
