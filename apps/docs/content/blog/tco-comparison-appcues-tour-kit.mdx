...
- Tour Kit Pro license at $9.99 one-time for advanced features
...
**No vendor lock-in.** Tour Kit is source-available under BSL 1.1, and each release converts to MIT on its Change Date. If you stop using it tomorrow, your flows are still React components that render without the library. There's nothing to migrate away from.

**Performance.** Tour Kit's core is under 23KB gzipped. Appcues' SDK adds 80-120KB to your bundle. At scale, that affects Core Web Vitals, which affects SEO, which affects acquisition cost. The performance tax compounds just like the subscription cost.
