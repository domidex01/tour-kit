...
<tbody>
<tr><td>Tour Kit</td><td>Web (responsive)</td><td>❌</td><td>❌</td><td>❌</td><td>✅ AA</td><td>Free in dev (BUSL-1.1)</td><td>Web-first teams wanting full UI control</td></tr>
...
Tour Kit is a headless onboarding library for React that ships its core at under 23KB gzipped with zero runtime dependencies. It doesn't have native mobile SDKs. What it does have: responsive web support with proper mobile breakpoint handling, WCAG 2.1 AA compliance baked in, and ten composable packages you install individually so you only ship what you use.

### Strengths

- Core bundle under 23KB gzipped, which matters on mobile web where [53% of users leave if a page takes over 3 seconds to load](https://developers.googleblog.com/the-modern-mobile-web-state-of-the-union/)
...
$0 for the core packages (MIT open source). Extended packages (surveys, adoption tracking, scheduling) available as a $9.99 one-time Pro license.
...
**If budget is the primary constraint**, UserGuiding at $69/month handles web basics. Tour Kit's free BUSL-1.1 core costs nothing in development and gives you more technical control.
...
It depends on your platform mix. For native mobile apps with web, Appcues offers the broadest framework coverage starting at $300/month. For web-first products, Tour Kit provides headless onboarding at under 23KB gzipped with WCAG 2.1 AA compliance for free. Pendo's free tier (500 MAU) works if you also need product analytics.
...
<script type="application/ld+json" dangerouslySetInnerHTML={{__html: JSON.stringify({
  ...
  "mainEntity": [
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit is source-available (BUSL-1.1, free in development) with responsive web support for mobile browsers. Pendo's free tier covers 500 monthly active users across web and mobile with native SDKs. For React Native specifically, react-native-onboard and react-native-onboarding-swiper are free open-source options."
      }
    },
    {
      "@type": "Question",
