...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>$0-$9.99 one-time + existing tool costs</td>
    </tr>
  </tbody>
</table>
...
A composable approach changes the math entirely. Tour Kit's core package ships at under 23KB gzipped with zero runtime dependencies. You install only the packages you need (tours, hints, checklists, surveys) and tree-shaking removes the rest. Loading a 300KB platform script versus an 23KB library isn't a marginal difference. On mobile connections, it's the gap between a sub-100ms Interaction to Next Paint and a visible delay.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Under 23KB, headless, works with your design system</td>
    </tr>
    <tr>
      <td>Analytics</td>
...
All-in-one onboarding platforms cost between $15,000 and $140,000 per year at 10,000 MAU. A composable stack built on Tour Kit (free in development, $9.99 one-time to ship) plus existing analytics and feature flag tools reduces onboarding tooling costs by 90% or more. Savings compound as MAU grows because composable tools don't charge per user.
