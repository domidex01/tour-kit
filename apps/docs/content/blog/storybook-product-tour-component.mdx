...
Tour Kit ships two layers: `@tourkit/core` at under 23KB gzipped handles tour logic, while `@tourkit/react` at under 12KB gzipped provides pre-styled components. That split creates two documentation targets for your Storybook. MDX pages with Doc Blocks let you render both the headless hook API and the styled wrapper on a single page, so consumers see exactly what each approach looks like.
