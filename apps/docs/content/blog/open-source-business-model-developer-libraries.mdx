...
I built [Tour Kit](https://usertourkit.com/) as a solo developer. Seventeen packages, source-available core, TypeScript-strict, the whole thing. Along the way I had to choose a business model that wouldn't alienate the developers I was trying to serve. Here's what I learned, and what I'd tell you if you were making the same choice today.
...
Tour Kit uses source-available licensing. Five packages are BUSL-1.1-licensed: `@tour-kit/core`, `@tour-kit/react`, `@tour-kit/hints`, plus `@tour-kit/vue` and `@tour-kit/svelte`. They give you everything you need to build product tours: hooks, providers, positioning, highlighting, keyboard navigation, WCAG 2.1 AA accessibility. Development, evaluation, testing, and CI are free with no key; production deployments need a $9.99 one-time licence key, and every version converts to MIT four years after release.

Nine extended packages are proprietary and require a Pro license: adoption tracking, analytics, announcements, checklists, media embedding, scheduling, surveys, AI, and license validation. These are features a solo developer building a side project doesn't need. They're features a product team onboarding 10,000 users per month does need.

The price is [$9.99 one-time](/pricing). Not per seat, not per month. One payment, lifetime access, all future updates.
...
2. **Never move a feature from free to paid.** Add new paid features. Never reclassify existing ones. Shepherd.js's switch from MIT to AGPL cost them community trust. React's BSD+Patents license cost Facebook an ecosystem revolt. Tour Kit's own move from MIT to BUSL-1.1 in v3.0 followed this rule literally: every version up to 2.1.0 stays MIT forever, and only new versions carry the new terms.
...
*Tour Kit is a source-available product tour library for React under BUSL-1.1. Every package is free while you build; serving them to end users needs a [licence key](https://usertourkit.com/). This article reflects the author's own experience choosing a business model — your situation may differ.*
