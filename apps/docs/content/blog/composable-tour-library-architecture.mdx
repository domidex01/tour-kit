...
Tour Kit takes a different approach. It ships 10 separate packages, each published independently, each tree-shakeable, each with its own TypeScript declarations. A basic tour pulls in `@tour-kit/core` (under 23KB gzipped) and `@tour-kit/react`. Need in-app surveys? Add `@tour-kit/surveys`. Need analytics? Add `@tour-kit/analytics` with a PostHog or Mixpanel plugin. You don't pay for what you don't use.
...
Tour Kit's core package targets under 23KB gzipped. The React rendering layer adds another 12KB. Checklists, surveys, announcements, analytics, media embeds, and scheduling are features that maybe 20% of users actually need. Forcing the other 80% to download that code felt wrong.
...
- **Core + React + Hints** are the foundation. BUSL-1.1 licensed (free in development). Every Tour Kit user installs at least core + react. Hints is separate because not every tour needs persistent hotspots.
...
**Skipping bundle size budgets.** Without explicit limits (core < 23KB, react < 12KB gzipped), package sizes creep upward with every feature addition. We enforce these as CI checks that fail the build.
...
4. **Set bundle size budgets and enforce them.** Tour Kit's quality gates (core < 23KB, react < 12KB, hints < 6.5KB gzipped) are CI checks, not aspirations. If a PR exceeds the budget, it fails.
...
Tour Kit's composable architecture benefits projects of any size because you only install what you need. A minimal setup (`@tourkit/core` + `@tourkit/react`, under 23KB gzipped combined) is smaller than most single-package tour libraries. The 10-package split means you never pay a bundle size penalty for features you don't use.
