...
Combined weight matters more than either number alone. Tour Kit's core ships at under 23KB gzipped. Adding PostHog's replay SDK (52.4KB) multiplies your onboarding JavaScript by 7x. Two strategies help:
...
- **Tour Kit** (BUSL-1.1): product tours and onboarding flows
...
Tour Kit doesn't bundle a session replay SDK, and that's deliberate. Tour Kit's core ships at under 23KB gzipped because it focuses on tour logic, not analytics infrastructure. The `@tourkit/analytics` package provides the event hooks (`onStepView`, `onStepComplete`, `onTourDismiss`) that feed into whatever replay tool your team already uses.
