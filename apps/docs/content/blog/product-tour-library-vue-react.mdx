...
Driver.js is the lightest cross-framework option at 83 kB unpacked (roughly 5 kB gzipped). It's pure vanilla JavaScript with no framework wrappers, which means you call it imperatively from Vue's `onMounted` or React's `useEffect`. Tour Kit's core is under 23 kB gzipped but currently supports only React. Shepherd.js is 656 kB unpacked and Intro.js is 874 kB unpacked.
...
<script type="application/ld+json">{`
{
  ...
  "mainEntity": [
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Driver.js is the lightest cross-framework option at 83 kB unpacked (roughly 5 kB gzipped). It is pure vanilla JavaScript with no framework wrappers, which means you call it imperatively from Vue's onMounted or React's useEffect. Tour Kit's core is under 23 kB gzipped but currently supports only React."
      }
    },
    {
      "@type": "Question",
