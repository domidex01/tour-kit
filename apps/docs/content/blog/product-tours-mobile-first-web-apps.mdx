...
Tour Kit's core package is under 23KB gzipped. That's small enough that even on a slow 3G connection (400KB/s effective throughput), the library loads in under 60ms after the initial chunk arrives. But your step content, images, and animations add up. Use `requestIdleCallback` to preload step content during browser idle time.
...
Lazy-load tour components with `React.lazy` so they don't block First Contentful Paint. Use CSS transitions instead of JavaScript animation loops. Render overlays in React portals with `position: fixed` to prevent layout shift. Tour Kit's core is under 23KB gzipped, adding under 60ms load time on slow 3G.
