...
Tour Kit's core package is under 23KB gzipped. That's small enough that even on a slow 3G connection (400KB/s effective throughput), the library loads in under 20ms after the initial chunk arrives. But your step content, images, and animations add up. Use `requestIdleCallback` to preload step content during browser idle time.
...
**Tour Kit** handles responsive positioning through Floating UI, renders in portals to avoid CLS, and gives you full control over step layout through headless components. The 23KB core means minimal mobile performance impact. One honest limitation: Tour Kit doesn't ship a built-in mobile layout mode or visual builder. You build touch-friendly patterns yourself using the headless primitives, which means more upfront work than a turnkey solution.
...
Lazy-load tour components with `React.lazy` so they don't block First Contentful Paint. Use CSS transitions instead of JavaScript animation loops. Render overlays in React portals with `position: fixed` to prevent layout shift. Tour Kit's core is under 23KB gzipped, adding under 20ms load time on slow 3G.
