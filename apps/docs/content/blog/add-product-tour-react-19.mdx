...
React Joyride ships opinionated tooltip UI at roughly 30KB gzipped. userTourKit is headless at under 23KB for the core. You get the tour logic (step sequencing, positioning, keyboard nav) and render tooltips with your own components. If you use shadcn/ui or Tailwind, that means zero style conflicts.

### Does adding a product tour affect performance?

userTourKit's core ships under 23KB gzipped with zero runtime dependencies. React Joyride weighs roughly 30KB and Shepherd.js roughly 25KB ([bundlephobia](https://bundlephobia.com), April 2026). userTourKit uses a single SVG spotlight instead of multiple DOM nodes, so layout shift isn't a concern.
