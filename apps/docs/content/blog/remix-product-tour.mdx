...
description: "Build a product tour in Remix with userTourKit. Covers loaders, nested routes, focus management, and tour state persistence across navigations."
primary_keyword: "remix product tour"
secondary_keywords: ["remix onboarding tutorial", "remix react tour", "react router v7 product tour"]
category: "Tutorials"
...
We built and tested this in a Remix 2.15 + React 19 + TypeScript 5.7 project running on Vite. The same patterns apply to React Router v7 in framework mode, since Remix merged into React Router v7 as of December 2024 ([Remix blog](https://remix.run/blog/merging-remix-and-react-router)). If you're on React Router v7 already, the only change is your import paths.
...
Then fire the action when the tour finishes:
...
Remix doesn't manage focus when routes change. The framework's own accessibility guide calls this out: "What element receives focus when the route changes? This is important for keyboard users" ([Remix docs](https://v2.remix.run/docs/guides/accessibility)). When your tour navigates between routes, keyboard users can lose their place.
...
Separately, a Remix 3 project exists that replaces React with a Preact fork and takes an HTML-over-the-wire approach ([LogRocket](https://blog.logrocket.com/remix-3-ditched-react/)). That version won't run React tour libraries at all. If you're evaluating Remix 3, you'll need a different approach to onboarding UX.
...
<table>
  <thead>
    <tr>
      <th>Feature</th>
      <th>Remix 2 / React Router v7</th>
      <th>Remix 3 (Preact fork)</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>React tour library support</td>
      <td>Full support</td>
      <td>None (no React)</td>
    </tr>
    <tr>
      <td>Loaders and actions</td>
      <td>Yes, route-bound</td>
      <td>Yes, similar model</td>
    </tr>
    <tr>
...
- Add conditional tours based on user role using a [loader that checks permissions](/blog/conditional-product-tour-user-role)
- Track tour analytics with `@tourkit/analytics` and [your analytics provider](/docs/analytics)
- Build an onboarding checklist that includes the tour as one task using `@tourkit/checklists`
- Add [Framer Motion animations](/blog/product-tour-framer-motion-animations) to the tooltip transitions
...
*Sources: [Remix + React Router merge announcement](https://remix.run/blog/merging-remix-and-react-router), [Remix accessibility guide](https://v2.remix.run/docs/guides/accessibility), [CodiLime Remix best practices](https://codilime.com/blog/project-in-remix-best-practices/), [LogRocket: Remix 3 ditched React](https://blog.logrocket.com/remix-3-ditched-react/), [Smashing Magazine: Product tours in React](https://www.smashingmagazine.com/2020/08/guide-product-tours-react-apps/). Data points date-stamped April 2026.*
