...
description: "Build an accessible React onboarding wizard with stepper UI using Tour Kit. Step-by-step TypeScript tutorial with focus management, ARIA roles, and analytics."
primary_keyword: "react onboarding wizard"
secondary_keywords: ["react stepper component onboarding", "wizard ui react", "react wizard tutorial"]
category: "Tutorials"
...
We tested five popular React stepper tutorials and found zero that implement `aria-current="step"` or manage focus on step transitions. Tour Kit takes a different approach. It gives you headless primitives for step sequencing, element highlighting, and scroll management while you keep full control of the rendering. The core ships at under 23KB gzipped with zero runtime dependencies (verified on [bundlephobia](https://bundlephobia.com/)). By the end of this tutorial, you'll have a working 4-step onboarding wizard with a stepper UI, proper ARIA roles, and optional analytics tracking.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Yes</td>
      <td>Yes</td>
      <td>No (opinionated UI)</td>
      <td>Yes</td>
    </tr>
    <tr>
      <td>ARIA support built-in</td>
      <td>Yes</td>
      <td>Minimal</td>
      <td>Partial</td>
      <td>Manual</td>
    ...
    <tr>
      ...
      <td>Yes (onStepChange, onComplete)</td>
      <td>No</td>
      <td>No</td>
      <td>Manual</td>
    </tr>
    <tr>
      <td>Works with shadcn/ui</td>
      <td>Yes</td>
      <td>Yes</td>
      <td>Conflicts</td>
      <td>Yes</td>
    ...
    <tr>
      <td>Best for</td>
      <td>Accessible onboarding with analytics</td>
      <td>Minimal stepper, tiny budget</td>
      <td>Material Design projects</td>
      <td>Learning, simple 2-3 step flows</td>
    </tr>
  </tbody>
</table>
...
Check the [Tour Kit documentation](https://tourkit.dev/docs) for the full API reference and more examples. The source code for this tutorial is on [GitHub](https://github.com/domidex01/tour-kit).
...
Tour Kit works with both React 18.2+ and React 19. The library uses hooks (`useTour`, `useStep`) that are compatible with React's concurrent features. The core package ships at under 23KB gzipped with zero peer dependencies beyond React itself.
...
Tour Kit's core adds under 23KB gzipped to your bundle, compared to MUI Stepper at roughly 80KB+. As the LogRocket blog notes, "adding a third-party package for every development requirement isn't always a good choice" ([source](https://blog.logrocket.com/add-stepper-components-react-app/)). Under 23KB for step management, focus handling, and ARIA support is a reasonable trade-off.
...
{JSON.stringify({
  ...
  "@type": "TechArticle",
  "headline": "How to build an onboarding wizard in React with stepper UI",
  "description": "Build an accessible React onboarding wizard with stepper UI using Tour Kit. Step-by-step TypeScript tutorial with focus management, ARIA roles, and analytics.",
  "author": {
    "@type": "Person",
    "name": "domidex01",
  ...
  "image": "https://tourkit.dev/og-images/react-onboarding-wizard.png",
  "url": "https://tourkit.dev/blog/react-onboarding-wizard",
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://tourkit.dev/blog/react-onboarding-wizard"
  },
  "keywords": ["react onboarding wizard", "react stepper component onboarding", "wizard ui react"],
...
{JSON.stringify({
  ...
  "itemListElement": [
    ...
    },
    {
      "@type": "ListItem",
      "position": 3,
      "name": "How to build an onboarding wizard in React with stepper UI",
      "item": "https://tourkit.dev/blog/react-onboarding-wizard"
    }
  ]
})}
</script>
...
{JSON.stringify({
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "What is a React onboarding wizard?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "A React onboarding wizard is a multi-step UI component that guides new users through an initial setup flow. Tour Kit implements this as a state machine with typed steps, lifecycle callbacks, and built-in focus management. Most implementations use a horizontal stepper indicator showing progress across 3-7 steps."
      }
    },
    {
      "@type": "Question",
      "name": "Does Tour Kit support React 19?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit works with both React 18.2+ and React 19. The library uses hooks (useTour, useStep) compatible with React's concurrent features. The core package ships at under 23KB gzipped with zero peer dependencies beyond React itself."
      }
    },
    {
      "@type": "Question",
      "name": "How is Tour Kit different from React Joyride for wizards?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "React Joyride is designed for tooltip-based product tours that highlight existing UI elements. Tour Kit handles both product tours and multi-step wizard flows, providing step sequencing, form validation hooks, and analytics callbacks that Joyride wasn't built for."
      }
    },
    {
      "@type": "Question",
      "name": "Does adding a stepper wizard affect performance?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit's core adds under 23KB gzipped to your bundle, compared to MUI Stepper which pulls in roughly 80KB+. Under 23KB for step management, focus handling, and ARIA support is a reasonable trade-off for most applications."
      }
    },
    {
      "@type": "Question",
      "name": "Can I use Tour Kit's wizard with shadcn/ui?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Yes. Tour Kit is headless — it manages step logic and provides hooks but doesn't render any UI. The asChild pattern through Tour Kit's UnifiedSlot lets you compose primitives directly with shadcn/ui components like Button, Card, and Input."
      }
    }
  ]
})}
