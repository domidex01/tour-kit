...
Bundle size alone doesn't tell you whether a library is worth using. A 3KB library with an AGPL license might cost you more than an 23KB one with MIT licensing and built-in accessibility. We scored each library across five dimensions:
...
<tbody>
<tr><td>Tour Kit (core)</td><td>&lt;23 KB</td><td>0</td><td>Yes</td><td>Strict</td><td>BUSL-1.1</td><td>WCAG 2.1 AA</td><td>Design system teams</td></tr>
...
<tr><td>OnboardJS</td><td>&#126;8-10 KB</td><td>0</td><td>Yes</td><td>Built-in</td><td>BUSL-1.1</td><td>Not documented</td><td>Flow orchestration</td></tr>
</tbody>
</table>
...
## 1. Tour Kit: best for headless tours under 23KB

Tour Kit ships 10 composable packages, but you only install what you need. Core weighs under 23KB gzipped with zero runtime dependencies. Add the React package and you're still under 35KB total. Every import tree-shakes cleanly because tsup outputs ESM with `sideEffects: false` in every package.json.
...
- Pro packages (adoption, analytics, announcements, checklists, media, scheduling, surveys) require a $9.99 one-time license

### Pricing

MIT core and React packages are free forever. Pro tier is $9.99 one-time with no per-seat fees, no monthly bill, no usage caps.
...
Onborda is purpose-built for Next.js App Router projects. Around 23KB gzipped with built-in TypeScript support and React 19 compatibility. It uses Framer Motion for animations, which is worth noting: if you're not already using Framer Motion, adding Onborda means adding its 30KB+ peer dependency too.
...
Intro.js is the smallest at roughly 4-5KB gzipped, but its AGPL-3.0 license restricts commercial use without a paid license. Among MIT-licensed options, Driver.js is lightest at approximately 5KB gzipped. Tour Kit's core ships under 23KB gzipped with WCAG 2.1 AA accessibility, making it the lightest accessible option.

### Can a lightweight tour library handle complex onboarding flows?

Tour Kit supports multi-step tours, conditional branching, progress persistence, and analytics through its modular package system. Core stays under 23KB, and each additional package adds capabilities incrementally. OnboardJS offers flow orchestration with branching but lacks DOM highlighting. Driver.js handles basic sequential tours only.
  ...
  "mainEntity": [
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Intro.js has the smallest raw gzipped size at roughly 4-5KB, but its AGPL-3.0 license restricts commercial use without a paid license. Among MIT-licensed options, Driver.js is the lightest at approximately 5KB gzipped. Tour Kit's core ships under 23KB gzipped with WCAG 2.1 AA accessibility included."
      }
    },
    {
      "@type": "Question",
