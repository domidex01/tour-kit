...
A React library takes the opposite approach. Tour Kit's core package ships at under 23KB gzipped with zero runtime dependencies. It doesn't fight your component tree. It *is* part of your component tree.
...
For teams with React developers, a library like Tour Kit provides equivalent core functionality (step tours, highlights, tooltips, checklists) at a fraction of the cost. Appcues starts at $299/month while Tour Kit's core is free and ships at under 23KB gzipped. The tradeoff is losing the visual builder and built-in analytics dashboard.
...
SaaS onboarding scripts inject 50-200KB of JavaScript on every page load, affecting Core Web Vitals scores. Tour Kit's core ships at under 23KB gzipped and tree-shakes so only imported code reaches the browser. For performance-sensitive applications, the bundle size difference directly impacts Interaction to Next Paint and Total Blocking Time.

### Is Tour Kit free or paid?

Tour Kit's core packages (`@tourkit/core`, `@tourkit/react`, `@tourkit/hints`) are BUSL-1.1-licensed and source-available. Extended packages like analytics and surveys require a Pro license. We're transparent about this: the best onboarding software being a library doesn't mean it has to be ours. React Joyride and Shepherd.js are solid MIT alternatives.
  ...
  "mainEntity": [
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "For teams with React developers, a library like Tour Kit provides equivalent core functionality (step tours, highlights, tooltips, checklists) at a fraction of the cost. Appcues starts at $299/month while Tour Kit's core is free and ships at under 23KB gzipped. The tradeoff is losing the visual builder and built-in analytics dashboard."
      }
    },
    {
      "@type": "Question",
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "SaaS onboarding scripts inject 50-200KB of JavaScript on every page load, affecting Core Web Vitals scores. Tour Kit's core ships at under 23KB gzipped and tree-shakes so only imported code reaches the browser. For performance-sensitive applications, the bundle size difference directly impacts Interaction to Next Paint and Total Blocking Time."
      }
    },
    {
      "@type": "Question",
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tour Kit's core packages (@tourkit/core, @tourkit/react, @tourkit/hints) are BUSL-1.1-licensed and source-available. Extended packages like analytics and surveys require a Pro license. React Joyride and Shepherd.js are solid MIT alternatives."
      }
    }
  ]
})}</script>
