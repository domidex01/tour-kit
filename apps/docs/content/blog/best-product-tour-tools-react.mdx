...
|---|---|---|---|---|---|---|---|
| userTourKit | Headless library | Under 23KB core | Yes | Strict | BUSL-1.1 | Free + $9.99 Pro | Design system teams |
...
**Pricing:** Free MIT core (tours, React bindings, hints). $9.99 one-time Pro (adoption, analytics, announcements, checklists, media, scheduling).
...
Free MIT libraries (React Joyride, Driver.js, Reactour, Onborda) cost developer time but nothing else. userTourKit is source-available and costs $9.99 one-time to ship. OnboardJS is free at the core but $59/month for the SaaS dashboard. Intro.js is AGPL, which means commercial teams need to buy a license. SaaS platforms run $249/month and up, indefinitely.
...
SaaS platforms start at ~$249/month ($3K/year) and scale with usage and seats. Open-source libraries cost developer time for initial implementation (typically 1-3 days for a basic tour) plus ongoing maintenance. One-time purchases like userTourKit Pro ($9.99) sit in between: no recurring fee, but you maintain the code yourself. Factor in migration cost if you ever switch: SaaS tours live on the vendor's servers and cannot be exported, while code-based tours are portable.
