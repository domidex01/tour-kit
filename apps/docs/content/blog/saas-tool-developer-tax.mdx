...
With a library like Tour Kit, the onboarding code lives in your repository. It uses your design system. It ships in your bundle at 23KB gzipped (not a 45KB external script). It doesn't break when you upgrade React. And it costs $9.99 once for the Pro tier, not $300-750/month scaling with your MAU count.
...
So we split it into 10 composable packages. Install `@tourkit/core` at under 23KB for basic tours. Add `@tourkit/surveys` only if you need NPS. Add `@tourkit/analytics` only if you need event tracking. The approach means you pay (in bundle size) only for what you use.
...
For teams with React developers, product tours fall into the "own in code" category. SaaS tools like Appcues ($279-750/month) inject 30-60KB of external JavaScript and charge per MAU. Libraries like Tour Kit ship at under 23KB gzipped, use your design system, and cost $9.99 once. The exception: teams without developers who need visual builders should buy SaaS.
