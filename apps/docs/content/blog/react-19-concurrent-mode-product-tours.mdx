...
Tour Kit's core ships at under 23KB gzipped and was built for React 19's rendering model from scratch. Every hook uses concurrent-safe patterns, with no stale closure bugs from interrupted renders and no torn reads from concurrent updates.
