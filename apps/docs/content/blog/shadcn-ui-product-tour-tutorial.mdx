...
Tour Kit is a headless React product tour library (core under 23KB gzipped) built specifically to pair with component libraries like shadcn/ui. It handles the hard parts (step sequencing, element highlighting, focus management, keyboard navigation, screen reader announcements) while you render tour steps with your existing shadcn/ui Card, Button, and Badge components. No style overrides. No `!important` hacks. Your design system stays intact because the tour literally uses your design system.
...
Tour Kit's core and React packages together add roughly 6KB gzipped to your production bundle for a typical 5-step tour. For context, a single shadcn/ui Dialog component (with its Radix primitives) adds about 23KB. Tour Kit tree-shakes unused exports, so you pay only for hooks and components you actually import.
...
<script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
  ...
  "mainEntity": [
    ...
    { "@type": "Question", "name": "Does adding a product tour affect my shadcn/ui app's bundle size?", "acceptedAnswer": { "@type": "Answer", "text": "Tour Kit's core and React packages together add roughly 6KB gzipped to your production bundle for a typical 5-step tour. For context, a single shadcn/ui Dialog component with Radix primitives adds about 23KB. Tour Kit tree-shakes unused exports, so you pay only for hooks and components you actually import." } },
