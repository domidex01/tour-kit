...
Tour Kit uses SVG overlay cutouts by default through the `useTourHighlight()` hook in `@tourkit/core`. The highlighting module adds roughly 1.2KB gzipped to the core's total 23KB bundle. The implementation follows the same pattern as Driver.js and Shepherd.js: a single SVG at document root with `evenodd` fill-rule path subtraction.
...
- Link from [How Tour Kit ships at 23KB with zero dependencies](/blog/tour-kit-8kb-zero-dependencies) (architecture decisions)
