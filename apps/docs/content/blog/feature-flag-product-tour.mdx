...
description: "Connect feature flags to product tours in React. Use LaunchDarkly, GrowthBook, or PostHog flags to target onboarding by cohort with type-safe code."
primary_keyword: "feature flag product tour"
secondary_keywords: ["feature flag onboarding", "launchdarkly product tour", "growthbook react onboarding", "conditional onboarding feature flag"]
category: "Use Cases"
...
<table>
<thead>
<tr><th>Provider</th><th>React SDK</th><th>Open source</th><th>SSR support</th><th>Real-time streaming</th><th>Self-host option</th></tr>
</thead>
<tbody>
<tr><td>LaunchDarkly</td><td>launchdarkly-react-client-sdk</td><td>No</td><td>Yes</td><td>Yes (SSE)</td><td>No</td></tr>
<tr><td>GrowthBook</td><td>@growthbook/growthbook-react</td><td>Yes (MIT)</td><td>Yes</td><td>Yes (SSE)</td><td>Yes</td></tr>
<tr><td>Flagsmith</td><td>flagsmith/react</td><td>Yes (BSD-3)</td><td>Yes</td><td>Yes</td><td>Yes</td></tr>
<tr><td>PostHog</td><td>posthog-js + posthog-react</td><td>Yes (MIT)</td><td>Yes</td><td>No</td><td>Yes</td></tr>
<tr><td>OpenFeature</td><td>@openfeature/react-sdk</td><td>Yes (Apache 2)</td><td>Provider-dependent</td><td>Provider-dependent</td><td>N/A (standard)</td></tr>
</tbody>
</table>

Other open-source proxy-based flag SDKs exist too, but most lack streaming support.

For Tour Kit integration, streaming matters most. When a flag changes mid-session (user upgrades plan, experiment assigns variant), SSE-capable SDKs from LaunchDarkly, GrowthBook, and Flagsmith push the update to the client. Tour Kit re-evaluates `when` callbacks on the next render, so the tour adapts without a page reload. Providers without streaming require a page refresh or manual re-fetch to pick up flag changes.
...
## Feature flag tools for product tour targeting

If you're picking a flag provider specifically for tour integration, here's the short version:

**GrowthBook** is the best fit for open-source-first teams. MIT license, self-hostable, SSE streaming, built-in experimentation, and strong TypeScript support. Pairs naturally with Tour Kit's architecture.

**LaunchDarkly** is the enterprise standard. It has the most granular targeting rules of any provider, plus audit logging and compliance features. The tradeoff: proprietary and priced per seat.

**PostHog** works if you already use it for analytics. Feature flags, experiments, and session replays in one tool. The gap: no real-time flag streaming, so flag changes require a page reload.

**OpenFeature** is worth watching. It's a CNCF standard that abstracts over any flag provider, so you can swap backends without changing tour code. The React SDK is young but maturing.

Tour Kit works with all of them. The `when` prop doesn't care where the boolean comes from.

## Limitations and honest tradeoffs

Tour Kit requires React 18+ and TypeScript. If your app runs on an older React version, this pattern won't work without upgrading. And Tour Kit doesn't have a visual builder (the way Appcues or Pendo do), so your product team can't drag-and-drop tour steps. The composition happens in code, which means developers own the implementation.

Feature flag-driven tours also add a dependency. If your flag service goes down and the SDK defaults to `false` for all flags, every flagged tour step disappears. Make sure your critical onboarding steps (welcome, core navigation) aren't behind flags. Only gate steps tied to optional or experimental features.

The community around Tour Kit is smaller than React Joyride or Shepherd.js. You won't find a Stack Overflow answer for every edge case yet. But the headless architecture means you're writing standard React code that any React developer can debug.
...
Wait for the flag SDK's "ready" state before mounting your tour component. GrowthBook exposes `gb.ready`, LaunchDarkly provides `ldClient.waitForInitialization()`, and Flagsmith has `flagsmith.init()`. Alternatively, evaluate flags in a Next.js Server Component and pass the results as props. This eliminates the async wait and prevents tour steps from flickering on screen.
...
- Link from [conditional-product-tour-user-role](/blog/conditional-product-tour-user-role): flag-based targeting as an evolution of role-based
- Link from [best-onboarding-tools-ab-testing](/blog/best-onboarding-tools-ab-testing): flag-driven experiments as a developer-owned approach
- Link from [managing-tour-state-zustand](/blog/managing-tour-state-zustand): flags as external state driving tour behavior
- Link to [track-product-tour-completion-posthog-events](/blog/track-product-tour-completion-posthog-events): measuring flag experiment results
- Link to [server-components-client-side-tours-boundary-problem](/blog/server-components-client-side-tours-boundary-problem): SSR flag evaluation pattern

**Distribution checklist:**
- Cross-post to Dev.to (canonical to usertourkit.com)
- Cross-post to Hashnode (canonical to usertourkit.com)
- Share on Reddit r/reactjs, angle: "pattern for connecting feature flags to onboarding tours"
- Share on Hacker News, angle: "type-safe feature flag-driven product tours in React"
...
```json
{
  ...
  "@type": "TechArticle",
  "headline": "Feature flag-driven tours: show the right tour to the right user",
  "description": "Connect feature flags to product tours in React. Use LaunchDarkly, GrowthBook, or PostHog flags to target onboarding by cohort with type-safe code.",
  "author": {
    "@type": "Person",
  ...
  "image": "https://usertourkit.com/og-images/feature-flag-product-tour.png",
  "url": "https://usertourkit.com/blog/feature-flag-product-tour",
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://usertourkit.com/blog/feature-flag-product-tour"
  },
  "keywords": ["feature flag product tour", "feature flag onboarding", "launchdarkly product tour", "growthbook react onboarding"],
