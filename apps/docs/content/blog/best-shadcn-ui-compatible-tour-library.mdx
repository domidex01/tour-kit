...
Tour Kit is the best shadcn/ui compatible tour library as of April 2026. It ships a headless core under 23KB gzipped that handles step sequencing, element highlighting, focus management, and keyboard navigation while you render tour steps with your own shadcn/ui components. No imported CSS, no style overrides, no `!important` hacks.
...
Tour Kit's core packages (@tourkit/core, @tourkit/react, @tourkit/hints) are BUSL-1.1 licensed and source-available. Extended packages (analytics, surveys, checklists, announcements) require a one-time Pro license at $9.99. No per-seat fees, no MAU pricing. Full breakdown at [usertourkit.com](https://usertourkit.com/).
