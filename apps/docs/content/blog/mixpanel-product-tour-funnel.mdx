...
Mixpanel's browser SDK adds roughly 30KB gzipped. Combined with Tour Kit's core (under 23KB gzipped), total impact is under 55KB. The `mixpanel.track()` calls are asynchronous and non-blocking. We measured under 2ms overhead per event in Chrome DevTools.
...
<script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
  ...
  "mainEntity": [
    ...
    },
    {
      ...
      "acceptedAnswer": { "@type": "Answer", "text": "Mixpanel's browser SDK adds roughly 30KB gzipped. Combined with Tour Kit's core (under 23KB gzipped), total impact is under 55KB. The mixpanel.track() calls are asynchronous and non-blocking, measuring under 2ms overhead per event." }
    },
    {
      "@type": "Question",
