...
**Tour libraries** guide users to that event. We built Tour Kit with headless tour components and `onStepComplete` / `onTourEnd` callbacks that pipe into whatever analytics platform you use, at under 23KB gzipped. Full disclosure: it's our project, so take that with appropriate skepticism. The library has a smaller community than React Joyride (603K weekly downloads) and no visual builder.
