...
Tour Kit's core ships at under 23 KB gzipped. That matters when your JS budget is 300 KB total.
...
<tbody>
<tr><td>Tour Kit core (~23 KB)</td><td>0.01 s</td><td>0.06 s</td><td>0.26 s</td></tr>
...
Tour Kit avoids this by shipping under 23 KB gzipped and deferring initialization until the tour triggers. Combined with React.lazy, the tour SDK contributes zero main-thread time during page load. We measured Tour Kit's initialization at under 2 ms on a Moto G4, the reference device for Lighthouse's mobile throttling.
...
1. **Ship under 50 KB for the tour chunk.** Tour Kit core + react is under 23 KB gzipped combined. That's a 0.16-second download on 3G. React Joyride's unpacked 498 KB takes over 4 seconds.
...
- **23 KB core, zero runtime dependencies.** No Popper.js, no Floating UI bundled by default. You add positioning only if you need it.
...
Service workers improve the experience for returning users by caching tour assets for offline or slow-connection use, but they aren't strictly required. Code splitting with React.lazy and keeping the tour bundle small are higher-priority wins. Tour Kit ships at under 23 KB gzipped, which downloads in 0.06 seconds on 3G even without caching.
...
- Link FROM [How Tour Kit ships at 23KB gzipped](/blog/tour-kit-8kb-zero-dependencies) → this article (low-bandwidth context)
