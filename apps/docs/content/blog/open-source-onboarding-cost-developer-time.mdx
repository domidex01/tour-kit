...
Tour Kit isn't free of tradeoffs. It requires React 18 or later, has no visual builder (you write JSX), and its community is smaller than React Joyride's. But the core is MIT-licensed, the bundle is under 23KB gzipped, and the extended packages cover what you'll need in month six without a custom build.
...
Tour Kit's core packages are BUSL-1.1-licensed and source-available. The difference is architecture: 10 composable packages covering tours, hints, analytics, checklists, announcements, and surveys. Free alternatives like React Joyride handle basic tours but require custom builds for everything else. Tour Kit's core stays under 23KB gzipped.
