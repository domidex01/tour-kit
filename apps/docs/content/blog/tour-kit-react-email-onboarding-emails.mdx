...
No. React Email templates render server-side to static HTML strings via the `render()` function. None of the email component code ships to the browser. The 6.5 MB `@react-email/tailwind` dependency lives in your server bundle only. Your client-side bundle stays exactly the same size, which matters for Tour Kit's under-23KB core footprint.
