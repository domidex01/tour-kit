...
Tour Kit renders all overlays through `createPortal` with a single persistent portal container. The reconciler diffs tooltip content as part of your component tree, and the mount/unmount cycle is controlled through CSS visibility toggling for steps. Bundle size: under 23KB gzipped for core + react packages combined.
