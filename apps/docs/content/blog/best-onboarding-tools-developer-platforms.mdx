...
Tour Kit is a headless onboarding library for React that gives you tour logic without prescribing any UI. The core ships at under 23 KB gzipped with zero runtime dependencies, providing hooks like `useTour()` and `useStep()` for positioning, state, keyboard navigation, and ARIA attributes. You render steps with your own components. If your developer platform uses shadcn/ui, Radix, or a custom design system, Tour Kit slots in instead of fighting it.

### Strengths

- Smallest bundle in this list: core < 23 KB, react < 12 KB gzipped
...
Free and open source (MIT) for core packages. Pro features (adoption tracking, scheduling, surveys) are $9.99 one-time.
...
**How much does bundle size matter?** Developer tools already load heavy components. Monaco Editor is ~2 MB. Tour Kit at &lt;23 KB won't move the needle. Appcues at 200 KB+ might.
...
Tour Kit is the best fit for developer tool platforms that need code-level control and design system integration. Ships at under 23 KB gzipped with WCAG 2.1 AA compliance. For teams wanting a managed platform, Frigade offers a code-first SDK for developer-led companies.
...
Source-available and open-source options: Tour Kit's core is BUSL-1.1-licensed and source-available (free in development), while Shepherd.js and Driver.js are MIT licensed and free. Tour Kit Pro is a one-time $9.99 payment. SaaS platforms start at $249-$300 per month, scaling with MAUs. For 5,000 MAUs, Appcues costs roughly $500-$800 per month.
