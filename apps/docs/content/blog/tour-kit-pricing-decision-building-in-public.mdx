...
description: "The real pricing decision behind Tour Kit's original $99 one-time license — since superseded by a three-tier ladder from $9.99. Unit economics, anchoring psychology, and why we left subscription revenue on the table."
...
> **Update, 11 September 2026 — the price in this post is no longer current.**
> Tour Kit moved off "$99 one-time for the Pro suite" to a three-tier ladder:
> **$9.99, $49.00 or $299.00** one-time, for 1, 5 or unlimited projects, with the
> *whole* library in every tier. The split is no longer free-core versus paid-extras —
> it is free in development versus a key in production, under BSL 1.1, with each
> published version converting to MIT on its Change Date. The reasoning below is
> what I thought at the time and I am leaving it up unedited; what changed my mind
> was the funnel, not the argument. See [pricing](/pricing) for the current model.
...
Tour Kit is a headless product tour library for React. Seventeen packages, BUSL-1.1 source-available core, TypeScript-first. The kind of thing most developers expect to be free. And most of it is. But the extended packages (surveys, scheduling, adoption tracking) need to pay for themselves or they stop getting maintained.
...
The core and React packages are BUSL-1.1 (free in development, licence key in production). What follows is about the Pro tier.
...
## Key decision 3: where to draw the source-available/Pro line
...
The BUSL-1.1 source-available packages give you a complete product tour system. Steps, navigation, highlighting, hints, persistence, keyboard navigation, screen reader support. A solo developer building their first onboarding flow never hits a paywall in development.
...
The source-available/Pro split landed without controversy. Not a single GitHub issue, not one angry tweet. I think it's because the boundary follows a natural line: individual developer needs (free) vs. organizational needs (paid). Nobody expects NPS survey infrastructure to be free. Nobody feels cheated that basic tour steps are source-available.
...
But the model stays: one-time payment, BUSL-1.1 core, Pro extensions. No monthly meter running between developers and their own code.
