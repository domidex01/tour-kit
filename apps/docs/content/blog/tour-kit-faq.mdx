...
Tour Kit is an open-source, headless product tour library for React. It ships as 10 composable packages, so you install only what you need. The core package is under 23KB gzipped with zero runtime dependencies. It handles tour logic (step sequencing, element targeting, scroll handling, focus management) without rendering any UI. You bring your own components. Tour Kit works with shadcn/ui, Radix, Tailwind, or any design system because it doesn't ship styles to fight against.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Architecture</td>
      <td>Bundle size?</td>
      <td>Core: &lt;23KB gzipped, zero dependencies</td>
    </tr>
    <tr>
      <td>Architecture</td>
      <td>TypeScript?</td>
    ...
    <tr>
      ...
      <td>Headless vs opinionated, 23KB vs 498KB unpacked</td>
    </tr>
    <tr>
      <td>Features</td>
      <td>Accessibility?</td>
...
Tour Kit's core package is under 23KB gzipped with zero runtime dependencies. The React package adds roughly 4KB. You install only the packages you need. Analytics, checklists, surveys, and other features are separate packages that tree-shake independently.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Bundle size</td>
      <td>&lt;23KB core, ~23KB with React</td>
    ...
    <tr>
      <td>License</td>
      <td>BUSL-1.1 (free in dev) + Pro ($9.99 one-time)</td>
      <td>MIT (free)</td>
    </tr>
    <tr>
      <td>Best for</td>
...
- **Licensing:** Shepherd's core is MIT, but "a paid license is required for commercial use" of advanced features ([Usertour](https://www.usertour.io/blog/product-tour-joyride)). Tour Kit is BUSL-1.1 for core packages (free in development, licence key in production), $9.99 one-time for Pro.
...
Tour Kit's core packages (`@tourkit/core`, `@tourkit/react`, `@tourkit/hints`) are BUSL-1.1-licensed: free in development, evaluation, testing, and CI, with a licence key required in production. The extended packages (analytics, adoption, checklists, surveys, announcements, media, scheduling) require a Pro license at $9.99 one-time per project, which also covers production use of the core.
...
<table>
  ...
  <tbody>
    <tr>
      <td>Tour Kit (core)</td>
      <td>BUSL-1.1</td>
      <td>Free in dev, licence key in prod</td>
    </tr>
    <tr>
      <td>React Joyride</td>
      <td>MIT</td>
...
Analytics is a Pro package ($9.99 one-time, includes all extended packages). The core tour functionality, including callback events you can wire to any analytics tool manually, is free.
...
- Bundle size matters (core is under 23KB gzipped)
...
Tour Kit's core packages (`@tourkit/core`, `@tourkit/react`, `@tourkit/hints`) are BUSL-1.1-licensed and source-available for commercial use. Extended packages for analytics, checklists, surveys, and media require a Pro license at $9.99 one-time per project. No per-user or per-MAU pricing.
    ...
    "mainEntity": [
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Tour Kit's core packages are BUSL-1.1-licensed and source-available for commercial use. Extended packages for analytics, checklists, surveys, and media require a Pro license at $9.99 one-time per project."
        }
      },
      {
        "@type": "Question",
