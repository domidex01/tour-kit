...
userTourKit is a headless product tour library structured as a monorepo itself. The core package (`@tourkit/core`) ships logic only, at under 23KB gzipped with zero runtime dependencies. The React package (`@tourkit/react`) adds hooks and renderless components. Neither package ships any CSS.
