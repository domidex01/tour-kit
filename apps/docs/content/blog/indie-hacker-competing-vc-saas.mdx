...
Tour Kit charges $9.99 one-time to ship, and nothing while you build. That pricing model isn't just competitive. It's un-matchable by companies that need to show 120% net revenue retention to their board.
...
Get started with [Tour Kit](https://usertourkit.com/), the source-available alternative to overpriced onboarding SaaS. Free while you build, $9.99 one-time to ship, no monthly tax. Check the [GitHub repo](https://github.com/user-tour-kit/tour-kit) or run `npm install @tourkit/core @tourkit/react` to try it.
...
Tour Kit inverts the VC-funded onboarding model. Instead of $249-$1,000/month with third-party scripts, it's a source-available npm package at 23KB gzipped with a $9.99 one-time licence. The tradeoff: Tour Kit requires React developers (no visual builder, no drag-and-drop), which limits the audience but deepens trust with the developers who do adopt it.
