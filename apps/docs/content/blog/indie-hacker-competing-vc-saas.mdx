...
Tour Kit charges $9.99 one-time for Pro features. The core library is MIT, free forever. That pricing model isn't just competitive. It's un-matchable by companies that need to show 120% net revenue retention to their board.
...
npm install @tourkit/core @tourkit/react  # 23KB gzipped, zero dependencies
# vs
# <script src="vendor-saas-tool.js">     # 200KB+, third-party cookies, "contact sales"
```

Get started with [Tour Kit](https://usertourkit.com/), the open-source alternative to overpriced onboarding SaaS. MIT licensed, $9.99 one-time for Pro features, no monthly tax. Check the [GitHub repo](https://github.com/user-tour-kit/tour-kit) or run `npm install @tourkit/core @tourkit/react` to try it.
...
Tour Kit inverts the VC-funded onboarding model. Instead of $249-$1,000/month with third-party scripts, it's an MIT-licensed npm package at 23KB gzipped with a $9.99 one-time Pro license. The tradeoff: Tour Kit requires React developers (no visual builder, no drag-and-drop), which limits the audience but deepens trust with the developers who do adopt it.
