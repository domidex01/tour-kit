...
The best Appcues alternative for developers depends on your constraints. Tour Kit gives React teams full design control at 8.1 KB gzipped with zero runtime dependencies, MIT-licensed, and $9.99 one-time for Pro features. Flows.sh is strong if you want a managed service with a developer-first API. React Joyride works for quick prototypes that don't need React 19. If bundle size matters most, Driver.js ships at 5 KB but requires DOM manipulation outside React's model.
...
**If bundle size is the only thing that matters:** Driver.js at ~5 KB gzipped. Smallest in the category. But it's vanilla JavaScript with direct DOM manipulation, which means fighting the virtual DOM in a React app with no component composition and no state management integration. Tour Kit's core is 23 KB and stays inside React's model.

**If you want open-source headless with managed analytics:** OnboardJS offers a similar headless approach to Tour Kit with built-in PostHog and Supabase plugins. The trade-off is $59/month recurring versus Tour Kit's $9.99 one-time.

## What we recommend

For a React team replacing Appcues, start with Tour Kit, free while you build. You get the core tour engine, step sequencing, keyboard navigation, and WCAG 2.1 AA accessibility out of the box. Want analytics, scheduling, surveys, and advanced components? The Pro license costs $9.99 one-time, which is still less than a single month of Appcues Essentials.
...
Appcues Essentials starts at $249 per month for up to 2,500 MAUs, which totals $2,988 per year minimum. Tour Kit's core is BUSL-1.1-licensed and source-available, with Pro at $9.99 one-time. React Joyride and Driver.js are both MIT and free. Shepherd.js is free under AGPL but requires a commercial license for proprietary apps.
