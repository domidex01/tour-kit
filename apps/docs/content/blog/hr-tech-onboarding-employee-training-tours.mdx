...
Tour Kit's core package is under 23KB gzipped with zero runtime dependencies. Adding `@tour-kit/react` and `@tour-kit/analytics` brings the total to roughly 15KB. Compare this to SaaS alternatives: Appcues adds 100KB+ and Pendo adds 200KB+ via external scripts. For enterprise HR portals that already ship heavy JavaScript bundles, the difference matters for page load times on employee devices.
