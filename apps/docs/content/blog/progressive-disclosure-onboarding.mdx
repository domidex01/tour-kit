...
Tour Kit's 10-package architecture is progressive disclosure applied to developer tooling. Install `@tourkit/core` (under 23KB gzipped) and `@tourkit/react` for Layer 1. Add `@tourkit/hints` when you need Layer 2. Bring in `@tourkit/checklists` and `@tourkit/announcements` for Layer 3. Each package tree-shakes independently.
