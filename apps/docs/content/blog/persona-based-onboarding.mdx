...
Tour Kit's core ships at under 23KB gzipped. Step configurations are plain objects, not components, so they're already small. For apps with rich persona-specific content (embedded videos, illustrations, interactive elements), lazy loading keeps payloads tight:
...
**Tour Kit** (headless, composable) handles tour rendering and step logic. Persona resolution lives in your code. Under 23KB gzipped. No visual builder, React 18+ only.
