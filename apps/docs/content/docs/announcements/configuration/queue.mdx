...
The queue is a `PriorityQueue<QueueItem>`. Order is determined by a comparator built from the configured `priorityOrder` and per-priority `weights`. To inspect or override that ordering, build the same comparator with `createComparator`.
