...
keywords:
  - tour kit troubleshooting
  - react tour library troubleshooting
  - usertourkit debugging
  - product tour not showing
  - tour kit common errors
  - react onboarding library issues
---
...
### Base UI render-prop mode

Tour Kit does not require an `@mui/base` peer. The optional Base UI rendering
path is handled by `UnifiedSlot`, which accepts render-prop children without
importing Base UI. If your app uses Base UI components directly, install the
current `@base-ui-components/react` package in your app.
