...
| Host | `LicenseProvider` mounted | License | Rendered |
|---|---|---|---|
| `localhost` / `127.0.0.1` / `*.local` | any | any | Children only, no badge |
| Other host | yes | valid pro | Children only, no badge |
| Other host | yes | invalid/expired/revoked | Children + badge + dev warning |
| Other host | yes | error + fresh cache | Children only, no badge |
| Other host | no | n/a | Children + badge + dev warning. No throw |
