...
title: Accessibility
description: >-
  Configure keyboard navigation, focus trapping, screen reader support, and
  reduced motion for WCAG 2.1 AA compliant tours
---
...
userTourKit is built with accessibility as a core principle, not an afterthought. Every component follows WCAG 2.1 AA guidelines to ensure your tours work for all users, including those using screen readers, keyboard navigation, or who have motion sensitivity.

<Callout type="info">
  userTourKit targets a **Lighthouse Accessibility Score of 100** and full WCAG 2.1 AA compliance.
</Callout>
...
userTourKit handles all of these concerns automatically.
...
userTourKit's `useFocusTrap` hook automatically:
...
userTourKit provides intuitive keyboard controls out of the box.
...
userTourKit ensures screen reader users understand what's happening at every step.
...
When tour steps change, userTourKit announces the new step to screen readers:
...
Users who experience motion sickness or have vestibular disorders can enable "reduced motion" in their operating system. userTourKit respects this preference.

### Automatic Detection

userTourKit automatically detects the `prefers-reduced-motion` media query:
...
Interactive elements in userTourKit meet minimum touch target requirements:
...
userTourKit uses CSS variables that ensure sufficient color contrast:
