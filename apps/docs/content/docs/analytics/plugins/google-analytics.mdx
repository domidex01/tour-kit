...
<Callout type="warn">
  This plugin requires gtag.js to be loaded on the page before the Tour Kit analytics provider initializes. Add the Google Analytics script tag to your HTML.
</Callout>
...
Tour Kit events stay well within these limits.
...
Tour Kit uses safe parameter names like `tour_id`, `step_id`, etc.
...
Then use the Tour Kit plugin as normal.
...
Ensure gtag loads before Tour Kit:
