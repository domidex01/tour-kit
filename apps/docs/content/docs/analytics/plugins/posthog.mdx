...
| Tour Kit Event | PostHog Event Name |
|----------------|-------------------|
...
<Callout type="info">
  Session replay is configured through PostHog's main SDK, not the Tour Kit plugin. The plugin only sends events.
</Callout>
...
Don't initialize PostHog twice - use the Tour Kit plugin instead of loading PostHog separately if you only need tour analytics:
