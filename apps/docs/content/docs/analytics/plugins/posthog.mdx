...
<TypeTable
  type={{
    ...
    autocapture: {
      type: 'boolean',
      default: 'false',
      description: 'Enable PostHog autocapture (not recommended with userTourKit)'
    },
    eventPrefix: {
      type: 'string',
      default: '"tourkit_"',
...
<Callout type="info">
  Session replay is configured through PostHog's main SDK, not the userTourKit plugin. The plugin only sends events.
</Callout>
