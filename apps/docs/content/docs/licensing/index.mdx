...
userTourKit uses a **source-available + Pro** licensing model:

- **Source-available packages** (`@tour-kit/core`, `@tour-kit/react`, `@tour-kit/hints`, plus the `@tour-kit/vue` and `@tour-kit/svelte` bindings) are BUSL-1.1-licensed: free for development, evaluation, testing, CI, and any non-production environment, with a licence key required in production.
- **Pro packages** (`@tour-kit/adoption`, `@tour-kit/analytics`, `@tour-kit/announcements`, `@tour-kit/checklists`, `@tour-kit/media`, `@tour-kit/scheduling`, `@tour-kit/ai`, `@tour-kit/surveys`) require a valid license key for production use. The same $9.99 one-time key also covers production use of the source-available packages.

<Callout type="info">
Source-available packages never require a license key in development, evaluation, testing, or CI. Production deployments need a key: use `@tour-kit/core`, `@tour-kit/react`, and `@tour-kit/hints` key-free until you ship.
</Callout>
...
No. `@tour-kit/core`, `@tour-kit/react`, and `@tour-kit/hints` are BUSL-1.1-licensed and work without any license key in development, evaluation, testing, and CI. Production deployments need a licence key.
...
No. When a non-empty `licenseKey` is set, development environments (`localhost`, `127.0.0.1`, `*.local`) are automatically bypassed and do not consume an activation slot. An empty key on localhost intentionally shows the unlicensed watermark — see [Development Mode](#development-mode).
