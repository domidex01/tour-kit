...
userTourKit's `@tour-kit/checklists` package (Pro, $9.99) provides task dependencies with circular dependency detection. Three completion types: manual, event-based, or custom check function. Progress calculation accounts for locked tasks. Everything shares core state with the tour system.
...
userTourKit's core is under 23KB gzipped. The React UI package is under 12KB. Hints: under 6.5KB. Each package is a separate entry point, so tree-shaking works. The headless architecture means you only ship the logic you use.
...
userTourKit's three core packages (core, react, hints) are MIT. No AGPL. No copyleft. The seven Pro packages cost $9.99 as a one-time purchase. No monthly fees, no MAU limits.
...
|---|---|---|
| Product tours | ✅ Built-in (core, BUSL-1.1) | ✅ Built-in (AGPL) |
| Hints/hotspots | ✅ Built-in (&lt;5KB, BUSL-1.1) | ✅ Built-in (viewport bugs reported) |
...
| Core bundle (gzipped) | &lt;23KB | ~10KB JS + CSS (not tree-shakeable) |
...
| License | BUSL-1.1 + $9.99 Pro | AGPL-3.0 + $9.99–$299.99 commercial |
...
userTourKit's three core packages (core, react, hints) are BUSL-1.1-licensed and source-available. The seven Pro packages cost $9.99 as a one-time purchase covering analytics, checklists, announcements, adoption tracking, media, and scheduling. No subscriptions. No MAU limits.
...
userTourKit's core is &lt;23KB gzipped. Intro.js is ~10KB minified JS plus a required CSS file, and the bundle isn't tree-shakeable. userTourKit's headless architecture ships only the logic you import.
