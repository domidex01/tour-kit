...
description: "Switching from Shepherd.js? Compare userTourKit vs Shepherd.js: bundle size, licensing (AGPL vs BUSL-1.1), React fit, and migration path. Find the right alternative for your stack in 2026."
...
userTourKit is a headless React library with tours, hints, checklists, announcements, analytics, and scheduling in a core bundle under 23KB gzipped, BUSL-1.1-licensed (free in development). Shepherd.js is a multi-framework tour library built on Svelte internals, dual-licensed under AGPL-3.0 and a $50-$300 commercial license. userTourKit fits React teams that want JSX-native tours, source-available licensing, and a small bundle. Shepherd.js is still the right choice when you need Vue, Angular, or vanilla JS support.
...
userTourKit ships a dedicated `@tour-kit/hints` package (under 6.5KB gzipped, BUSL-1.1 licensed) with `HintHotspot` for pulsing indicators and `HintTooltip` for on-hover/click content. Each hint dismisses independently and persists across page loads via the storage adapter. Not a workaround bolted on top of the tour system.
...
userTourKit's core bundles at under 23KB gzipped. React package: under 12KB. Hints: under 6.5KB. Because userTourKit is native React, it shares your existing runtime.
...
userTourKit is free while you build, and BSL 1.1 carries no copyleft obligation. A licence costs $9.99 one-time and covers every package. Not recurring.
...
| Core bundle (gzipped) | Under 23KB | ~25-40KB (includes Svelte runtime) |
| Multi-framework | React only | React, Vue, Angular, Ember, vanilla |
| Positioning engine | Floating UI | Floating UI |
| License | BUSL-1.1 (source-available) | AGPL-3.0 or $50-$300 commercial |
...
And if AGPL is a non-starter at your company, userTourKit's BUSL-1.1 source-available tier removes the conversation entirely.
...
The core library, React bindings, and hints are all free under the MIT license. The Pro tier costs $9.99 one-time (not recurring) and adds seven packages: adoption tracking, analytics, announcements, checklists, media embedding, scheduling, and more. No MAU limits on either tier.
...
userTourKit's core is under 23KB gzipped; the React package is under 12KB. Shepherd.js bundles at 25-40KB gzipped, including compiled Svelte runtime. A React project using Shepherd ships two UI runtimes.

### Is Shepherd.js open source?

Yes, but under AGPL-3.0 since October 2024. Any commercial use requires a $50-$300 lifetime license. The AGPL's network use clause means SaaS companies must either open-source their application or purchase a license. userTourKit uses BSL 1.1, which has no such requirement.
