...
It's also the right call when performance budgets are strict (&lt;8KB core versus an undisclosed CDN payload), when WCAG 2.1 AA compliance matters (Lighthouse 100), or when content must render without depending on a third-party CDN. And the $9.99 one-time price speaks for itself at any MAU scale.
...
userTourKit's core is &lt;8KB gzipped, React components add &lt;12KB, hints add &lt;5KB. Userflow loads a ~15KB stub that fetches the full SDK from its CDN asynchronously. Userflow doesn't disclose the total runtime payload size.
