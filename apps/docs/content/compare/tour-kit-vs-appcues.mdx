...
userTourKit is a headless React library offering tours, hints, checklists, announcements, and five more packages in a &lt;23KB core bundle for a $9.99 one-time fee. Appcues is a no-code onboarding platform starting at $750/month. userTourKit suits React teams wanting code ownership and tiny bundles. Appcues is better when non-technical PMs need to build tours without developers.
...
userTourKit's `@tour-kit/hints` package (&lt;6.5KB gzipped, BUSL-1.1 licensed) provides `HintsProvider`, `HintHotspot`, `HintTooltip` components. Each hint has independent dismissal state persisted through storage adapters. The `useHint()` hook returns `isVisible`, `isDismissed`, `show`, `hide`, `dismiss` for programmatic control.
...
userTourKit's `@tour-kit/scheduling` ($9.99 Pro) handles time-based display: start/end dates, time-of-day windows, day-of-week rules, recurring patterns, and blackout periods with IANA timezone support. The `ScheduleGate` component conditionally renders children based on schedule state.
...
userTourKit is source-available under BUSL-1.1 and free while you build — core, React bindings, hints, analytics, announcements, checklists, adoption tracking, media embedding and scheduling. A $9.99 one-time licence covers serving them to end users. Three-year total cost of ownership at 10,000 MAUs: Appcues runs $90,000 to $120,000+. userTourKit is $9.99.

## Side-by-side comparison table

| Feature | userTourKit | Appcues |
|---|---|---|
| Product tours | ✅ Built-in (core, BUSL-1.1) | ✅ WYSIWYG builder |
| Hints/hotspots | ✅ Built-in (&lt;5KB, BUSL-1.1) | ✅ Pins |
| Onboarding checklists | ✅ Pro ($9.99 one-time) | ⚠️ Grow plan only ($750+/mo) |
| Announcements | ✅ Pro ($9.99 one-time) | ✅ Built-in |
...
| Core bundle (gzipped) | &lt;23KB | Opaque (multi-phase CDN) |
...
| Pricing | Free + $9.99 one-time Pro | $750/mo+ (MAU-based) |
...
And the math: if you're a startup with 5,000 MAUs spending $3,500/year on Appcues Essentials (now likely more on Grow), the $9.99 one-time Pro license pays for itself before your first monthly invoice arrives.
...
**Is userTourKit free?**
The core package, React bindings, and hints are BUSL-1.1 licensed and source-available (free in development, licence key in production). The Pro upgrade ($9.99, one-time payment) adds analytics, announcements, checklists, adoption tracking, media embedding, and scheduling. No MAU limits. No recurring fees.

**What is the difference between userTourKit and Appcues?**
userTourKit is a code-first React component library. Appcues is a no-code SaaS platform with a visual builder. userTourKit renders inside your React tree using your design tokens; Appcues injects external DOM elements and iframes over your application. userTourKit costs $9.99 once, Appcues starts at $750/month.
...
userTourKit's core is &lt;23KB gzipped, the React package is &lt;12KB, and hints is &lt;6.5KB. Appcues doesn't publish its bundle size. Their multi-phase CDN loading (bootstrap, SDK, API calls, flow content) makes the total payload opaque and unmeasurable with standard tools like Bundlephobia.
...
No. Appcues is a proprietary SaaS product. Your onboarding flows are stored in Appcues' cloud infrastructure. userTourKit's source-available tier packages are BUSL-1.1 licensed, and the source code is publicly available.
...
For React teams that own their frontend, userTourKit gives you onboarding components that match your design system, pass WCAG 2.1 AA with Lighthouse 100, and cost $9.99 total. Appcues makes sense when PMs need to ship without developers, when mobile onboarding matters, or when enterprise compliance is non-negotiable. Your mileage may vary depending on team size and technical capacity.
