...
userTourKit's `@tour-kit/hints` package (&lt;6.5KB gzipped, MIT licensed) provides `HintsProvider`, `HintHotspot`, `HintTooltip` components. Each hint has independent dismissal state persisted through storage adapters. The `useHint()` hook returns `isVisible`, `isDismissed`, `show`, `hide`, `dismiss` for programmatic control.
...
userTourKit's free tier (MIT) includes core, React bindings, and hints. The $9.99 one-time Pro license adds analytics, announcements, checklists, adoption tracking, media embedding, and scheduling. Three-year total cost of ownership at 10,000 MAUs: Appcues runs $90,000 to $120,000+. userTourKit is $9.99.
...
**Is userTourKit free?**
The core package, React bindings, and hints are MIT licensed and free forever. The Pro upgrade ($9.99, one-time payment) adds analytics, announcements, checklists, adoption tracking, media embedding, and scheduling. No MAU limits. No recurring fees.
