---
title: "userTourKit vs UserGuiding: which product tour tool should you choose in 2026?"
description: "Compare userTourKit vs UserGuiding for product tours. See features, bundle size, pricing, and accessibility side-by-side. Find which fits your React stack in 2026."
...
userTourKit is a headless React library offering tours, hints, checklists, announcements, analytics, and scheduling in a &lt;23KB core bundle for a $9.99 one-time fee. UserGuiding is a no-code product adoption platform starting at $174/month with a visual Chrome Extension builder. userTourKit suits React teams wanting code ownership and tiny bundles with WCAG 2.1 AA baked in. UserGuiding is better when product managers need to ship onboarding flows without developer involvement.
...
userTourKit's `@tour-kit/analytics` ($9.99 Pro) takes a plugin approach. Five built-in plugins cover PostHog, Mixpanel, Amplitude, Google Analytics, and a console logger. Writing a custom plugin is three lines of code: implement `name` and `track`, then pass it to `AnalyticsProvider`.
...
userTourKit's `@tour-kit/scheduling` ($9.99 Pro) handles time-based display with IANA timezone support, four business hours presets (US Standard, US Tech, UK Standard, Australia Standard), recurring patterns, and blackout windows. The `ScheduleGate` component conditionally renders children only when a schedule is active. `useSchedule` returns the current status and reason (`outside_hours`, `blackout`, `wrong_day`), so you can show users why content isn't visible yet.

### Adoption tracking

UserGuiding offers basic feature adoption insights through their analytics, showing how users interact with onboarding elements. Their "Product Adoption" dashboard tracks activation rates and feature discovery. Available on the Professional plan ($249/month). The tracking is limited to UserGuiding-created elements — it doesn't measure organic feature usage in your application code.

userTourKit's `@tour-kit/adoption` ($9.99 Pro) measures feature usage against configurable adoption criteria. The adoption calculator compares actual usage count vs required threshold, time since first use vs adoption window, and recency of use vs churn threshold. A nudge scheduler respects cooldown periods and fires on time-based, usage-based, or event-based triggers. Components like `IfNotAdopted` conditionally render badges or prompts for users who haven't adopted a feature. `AdoptionDashboard` gives admins a visual overview of adoption metrics across features.

UserGuiding tracks adoption of its own onboarding elements. userTourKit tracks adoption of your actual product features. That distinction matters if you want to know whether users are using a feature organically, not just whether they clicked through the guide about it.

### Media embedding

UserGuiding supports YouTube and Vimeo embeds in guides and modals. Their visual builder handles video insertion. No support for Loom, Wistia, native video files, GIF players, or Lottie animations. No programmatic API for media management.
...
For basic YouTube or Vimeo embeds in a visual builder, UserGuiding works fine. userTourKit covers seven platforms with code-level control and built-in accessibility handling.
...
userTourKit ships WCAG 2.1 AA by default with a Lighthouse Accessibility score of 100. Focus trapping within tour cards uses `useFocusTrap`. Screen reader announcements use `aria-live` (polite or assertive). Keyboard navigation supports arrow keys for next/prev, Escape to exit, and Tab for focus cycling.
...
userTourKit's core bundle is under 23KB gzipped. The React package is under 12KB. Hints are under 6.5KB. Everything tree-shakes, so you ship only the code your application actually uses. There's no external runtime, no network requests to a vendor CDN. And no third-party script can push a bad update to your production environment at 2 AM on a Tuesday.
...
userTourKit is React 18+ only. That's a real limitation if your app runs Vue or Angular. Within the React ecosystem, though, the integration is deep. Hooks for tour state. Context providers for multi-tour management. Router adapters for Next.js (both App Router and Pages Router) plus React Router v6+.
...
userTourKit's core, React, and hints packages are BUSL-1.1-licensed and source-available. The Pro tier costs $9.99 one-time and covers adoption, analytics, announcements, checklists, media, and scheduling. Zero MAU limits. Zero recurring charges.

Over three years at 5,000 MAU on UserGuiding's Growth plan, a team would spend roughly $15,084. userTourKit costs $9.99 regardless of whether you have 100 users or 100,000.

## Side-by-side comparison table

| Category | userTourKit | UserGuiding |
|---|---|---|
| Product tours | ✅ Built-in (core, BUSL-1.1) | ✅ No-code visual builder |
| Hints/hotspots | ✅ Built-in (&lt;5KB, BUSL-1.1) | ✅ Visual builder (20 cap on Starter) |
| Onboarding checklists | ✅ Pro ($9.99 one-time) | ✅ Built-in (2 cap on Starter) |
...
| Adoption tracking | ✅ Pro (usage + nudges) | ⚠️ Basic (UserGuiding elements only) |
...
| Core bundle (gzipped) | &lt;23KB | Undisclosed (external script) |
...
| Pricing | Free + $9.99 one-time Pro | $174-$349+/mo (annual billing) |
...
UserGuiding also makes more sense when you need NPS surveys, a resource center, and an AI-powered help assistant bundled into one subscription. Building those from scratch alongside userTourKit would take meaningful engineering time. For product-led growth teams where the PM owns onboarding and the engineering team has higher priorities, UserGuiding's all-in-one platform is the more practical choice.

If your application isn't built with React, userTourKit isn't an option at all. UserGuiding works with any web stack.

## When userTourKit is the better fit

userTourKit wins when your team has frontend developers, your product is a React application, and you care about design fidelity or performance budgets. Long-term cost matters too.
...
The cost math is straightforward. If you're a 5,000 MAU company planning to use onboarding for the next three years, UserGuiding Growth costs roughly $15,084. userTourKit costs $9.99. That's a 99.3% cost reduction. Your mileage may vary (you'll spend engineering hours on implementation), but for most React teams the total cost of ownership still favors the library approach.

## Migration path from UserGuiding to userTourKit

Migrating from UserGuiding to userTourKit is a rebuild, not a port. The two tools have different architectures and there's no export format to convert. Here's the practical path:
...
**2. Install userTourKit packages.**

```bash
npm install @tour-kit/core @tour-kit/react @tour-kit/hints
```

**3. Convert guides to userTourKit steps.** Each UserGuiding guide becomes a `Tour` component with typed step configs:
...
**5. Run both tools in parallel for a week.** Show userTourKit to internal users or a small percentage of traffic. Compare completion rates. Then remove the UserGuiding script tag.
...
**Is userTourKit free?**
userTourKit's core, React bindings, and hints package are source-available under the BUSL-1.1 licence (free in development). The Pro tier costs $9.99 one-time (not monthly, not annual) and adds adoption tracking, analytics, announcements, checklists, media, and scheduling. No MAU limits on any tier.

**What is the difference between userTourKit and UserGuiding?**
userTourKit is a headless React library that developers integrate directly into application code. UserGuiding is a no-code SaaS platform with a visual Chrome Extension builder. userTourKit gives full design and performance control; UserGuiding gives speed-to-publish without developer involvement.

**Can I migrate from UserGuiding to userTourKit?**
...
userTourKit supports React 18 and 19. Built-in router adapters handle Next.js App Router, Next.js Pages Router, and React Router v6+. The `useNextAppRouter` hook connects userTourKit's navigation to Next.js routing.

**What is the bundle size of userTourKit vs UserGuiding?**
userTourKit's core is under 23KB gzipped, the React package under 12KB, and hints under 6.5KB. UserGuiding's external script size isn't publicly disclosed. userTourKit tree-shakes so you ship only the code you use; UserGuiding loads its full runtime on every page.
...
No. UserGuiding is proprietary SaaS. userTourKit's core packages (core, React, hints) are BUSL-1.1-licensed and source-available. The Pro packages are proprietary with a one-time $9.99 license.

**Which is better for enterprise?**
Depends on your enterprise. UserGuiding offers SOC2/HIPAA/GDPR compliance, SAML SSO, and dedicated CSMs on the Enterprise plan (custom pricing). userTourKit runs in your own infrastructure, so compliance is your responsibility, but you control the data. For enterprises with strong engineering teams, userTourKit's self-hosted model can simplify compliance by keeping user data out of a third-party platform entirely.

## Final verdict

For React teams with frontend developers, userTourKit gives you better performance (&lt;23KB vs. an undisclosed external script), real accessibility (WCAG 2.1 AA, Lighthouse 100), full design control, and a 99%+ cost reduction over three years. For product managers without developer support who need onboarding live today, UserGuiding's visual builder is genuinely faster to ship. The choice comes down to whether onboarding is an engineering concern or a marketing one. We built userTourKit, so factor that bias in.
