...
userTourKit ships a separate `@tour-kit/hints` package (under 6.5KB gzipped) for persistent, non-sequential hotspots. Each hint has its own dismiss state and persists across page loads. The component renders a pulsing beacon on the target element, with a tooltip on hover or click.
...
For teams embedding video walkthroughs in their tours, userTourKit removes the per-platform boilerplate. The trade-off is the same $9.99 Pro cost — if you only need one YouTube embed, a raw iframe is simpler.
...
userTourKit's core ships under 23KB gzipped. The React binding adds under 12KB. Hints: under 6.5KB. Because userTourKit is headless, you aren't shipping UI code you'll override anyway. The core contains only logic (hooks, state management, position calculation, storage adapters) and your existing design system components handle the rendering.

For context, 23KB gzipped for the core is smaller than most icon libraries. On a 3G mobile connection, that's roughly 100ms of additional load time versus several hundred for a larger bundle.
...
Only Reactour is fully MIT here: it is 100% free with no paid tier. userTourKit's core is BUSL-1.1-licensed source-available (free in development, licence key in production).

userTourKit's source-available tier (BUSL-1.1) includes `@tour-kit/core`, `@tour-kit/react`, and `@tour-kit/hints`. Seven Pro packages cover the rest: adoption, analytics, announcements, checklists, media, scheduling, plus licensing. All for $9.99 one-time. No monthly fees, no MAU limits.

If all you need is basic tours, Reactour costs nothing. But once you need analytics, checklists, or scheduling, userTourKit's $9.99 one-time fee beats building those features yourself. It also undercuts SaaS onboarding platforms at $300+/month by a wide margin.
...
| Core bundle (gzipped) | Under 23KB | ~98.5KB unpacked |
| React 18 support | ✅ | ✅ |
...
**Is userTourKit free?**
The core library, React components, and hints package are BUSL-1.1-licensed and source-available. Seven Pro packages (analytics, checklists, announcements, adoption, media, scheduling, licensing) cost $9.99 as a one-time purchase. No subscriptions, no MAU limits.
...
userTourKit's core is under 23KB gzipped, the React package under 12KB gzipped, and hints under 6.5KB gzipped. Reactour's `@reactour/tour` package has an unpacked size of 98.5KB, though the gzipped transfer size is smaller.
