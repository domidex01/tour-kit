...
userTourKit ships `@tour-kit/hints` (under 6.5KB gzipped, BUSL-1.1 licensed) with `HintHotspot` for pulsing indicators and `HintTooltip` for on-hover/click content. Each hint dismisses independently and persists across page loads. Non-sequential by design, not a hack on top of the tour system.

### Checklists and onboarding flows

Not available in Onborda. No task tracking, no progress persistence.

userTourKit's `@tour-kit/checklists` package (Pro, $9.99 one-time) handles task dependencies with circular dependency detection. Three completion types: manual, event-based, custom check functions. The `ChecklistPanel` component gives you a working onboarding checklist without stitching together multiple npm packages.
...
userTourKit's core bundles at under 23KB gzipped. React package: under 12KB. Hints: under 6.5KB. Animations use CSS by default, with bring-your-own-library as an option.
...
Onborda is fully MIT with no paid tier. userTourKit's source-available tier (core, react, hints) is BUSL-1.1 (free in development, licence key in production), while the Pro packages cost $9.99 as a one-time purchase.
...
| Core bundle (gzipped) | &lt;23KB | ~5-15KB + 32KB Framer Motion |
...
**Is userTourKit free?**
The core packages (tours, hints, react components) are BUSL-1.1 licensed and source-available. Pro packages (analytics, checklists, announcements, media, scheduling) cost $9.99 one-time. No subscriptions, no per-seat fees.
...
userTourKit's core is under 23KB gzipped. The react package is under 12KB. Onborda's own code is roughly 5-15KB, but it requires Framer Motion (approximately 32KB gzipped) as a mandatory dependency.
