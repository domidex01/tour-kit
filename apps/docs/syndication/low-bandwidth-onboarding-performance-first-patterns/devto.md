---
title: "Your product tour SDK is killing onboarding on 3G — here's how to fix it"
published: false
description: "A 5 MB JS file takes 40 seconds on 3G. Most tour libraries ship heavy SDKs that never load on slow networks. Here are the patterns that actually work: lazy loading, adaptive rendering, service worker caching, and strict bundle budgets."
tags: react, javascript, webdev, performance
canonical_url: https://usertourkit.com/blog/low-bandwidth-onboarding-performance-first-patterns
cover_image: https://usertourkit.com/og-images/low-bandwidth-onboarding-performance-first-patterns.png
---
...
| Tour asset | Caching strategy | Why |
|---|---|---|
| Tour JS bundle (code-split chunk) | Cache-First | Changes only on deploy. Serve from cache, update in background. |
| Step text / locale strings | Stale-While-Revalidate | May change between A/B variants. Serve cached, fetch updated version. |
| Step images / media | Cache-First with size limit | Large assets. Cache on first view, skip pre-caching on slow connections. |
| Tour configuration (step order, targeting rules) | Network-First | Must reflect latest targeting logic. Fall back to cache if offline. |
...
- **Tree-shakeable exports.** Unused hooks and utilities are eliminated at build time. [Our tree-shaking deep-dive](https://usertourkit.com/blog/tree-shaking-product-tour-libraries) covers what actually gets removed.
