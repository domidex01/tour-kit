...
I built a headless product tour library (Tour Kit) and wanted to map the landscape. The phrase "headless onboarding library" returns almost nothing useful — the category barely exists.

After testing 7 libraries in a React 19 project, the space breaks into two groups: purpose-built onboarding tools (Tour Kit, OnboardJS) and headless UI primitive libraries (Radix, React Aria, Base UI, Ark UI, Headless UI) whose popovers and dialogs become tour building blocks.
...
Disclosure: Tour Kit is my project, listed at #1 in the article. I've tried to be fair with the evaluation — every data point is verifiable against npm and GitHub.
