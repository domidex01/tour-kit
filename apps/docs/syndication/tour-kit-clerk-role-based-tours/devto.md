---
title: "Build role-based product tours with Clerk + Tour Kit (TypeScript)"
published: false
description: "Wire Clerk's useOrganization hook into Tour Kit's when prop so admins, members, and custom roles each see only the onboarding steps relevant to their permissions. Full TypeScript code included."
tags: react, typescript, webdev, tutorial
canonical_url: https://usertourkit.com/blog/tour-kit-clerk-role-based-tours
cover_image: https://usertourkit.com/og-images/tour-kit-clerk-role-based-tours.png
---
...
Clerk serves over 300,000 applications as of early 2026, with a free tier covering 50,000 MAU and Pro starting at $0.02/MAU after that. Auth0 charges $0.07/MAU after just 7,500 free users. For a 40,000-user SaaS app, Clerk costs $0 while Auth0 runs roughly $2,275/month. Pair that with Tour Kit's <8KB gzipped core (also free), and you have a role-based onboarding system at zero cost for most startups.
...
You could build role filtering from scratch with React Context. We covered that in our [conditional tours by user role](https://usertourkit.com/blog/conditional-product-tour-user-role) tutorial. But if you're already using Clerk, why maintain a separate role system? Clerk is the source of truth.
...
Focus management and keyboard navigation ([keyboard-navigable tours guide](https://usertourkit.com/blog/keyboard-navigable-product-tours-react)) work unchanged with filtered steps. The `when` prop removes steps before the tour engine processes them.
