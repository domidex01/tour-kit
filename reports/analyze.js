...
if (!path) {
  console.error('usage: analyze.js <report.json>')
  process.exit(1)
}
...
for (const c of sortedCats) {
  console.log(
    `  ${c.score.toString().padStart(3)}  ${c.id.padEnd(22)} pass:${c.pass} warn:${c.warn} fail:${c.fail}`
  )
}
...
const printTop = (map, label, limit = 15) => {
  console.log(`\nTop ${label}:`)
  const top = [...map.entries()].sort((a, b) => b[1].count - a[1].count).slice(0, limit)
  for (const [rule, e] of top) {
    console.log(`  [${e.count.toString().padStart(3)}]  ${rule}`)
    console.log(`         ${e.sampleMsg}`)
    const urls = [...e.sampleUrls]
      .slice(0, 2)
      .map((u) => u.replace('https://usertourkit.com', ''))
      .join(', ')
