...
- db05873: Add `@tour-kit/analytics/engine`, a React-free subpath exposing the tracker and
...
- 6653ba1: Remove two typed-but-dead public fields that promised behavior the runtime never delivered: `TourEvent.userProperties` (no production consumer) and `AnalyticsConfig.offlineQueue` (no offline-queue logic exists — the tracker batches only by `batchSize`).
