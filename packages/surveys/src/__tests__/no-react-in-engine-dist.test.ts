/**
 * v3 Phase 3 — `@tour-kit/surveys/engine` ships no React.
 *
 ...
 *     rule that replaces it is `../types`: `src/types/survey.ts` imports
 *     `react` and `types/question.ts` reaches `@tour-kit/media`, so ONE `import type … from '../types'`
 ...
 * `react-dom`, `@tour-kit/analytics` and `@radix-ui/react-slot` are here and in
 * no control: nothing in this package imports them, so no closure can
 * demonstrate them — but they are React-side specifiers and forbidding them is
 * still right.
 */
const FORBIDDEN = [
  'react',
...
/**
 * The subset of FORBIDDEN_RAW this package's main closure can actually
 * demonstrate. `@tour-kit/analytics` and `@radix-ui/` are forbidden but absent
 * from surveys entirely, so a control asserting them would red a correct build.
 */
...
describe('@tour-kit/surveys/engine ships no React', () => {
  ...
  it.skipIf(!distExists())('CONTROL — the raw-literal layer trips on the main closure too', () => {
    ...
    for (const needle of FORBIDDEN_RAW_CONTROL) {
  ...
  it.skipIf(!distExists())("'use client' lands on the React entries only", () => {
    // §0 C12: this package stamps TWO entries. `headless.ts` re-exports the
    // provider, so the binding rewrite changed that closure directly and it is
    // asserted here rather than assumed.
    expect(startsWithClientDirective(P.engineJs)).toBe(false)
    expect(startsWithClientDirective(P.engineCjs)).toBe(false)
    for (const name of ['index', 'headless']) {
  ...
  it('every source file the engine barrel reaches is React-free and avoids ../types', () => {
    ...
    expect(files.some((f) => f.endsWith('lib/surveys-engine/reducer.ts'))).toBe(true)
    expect(files.some((f) => f.endsWith('lib/surveys-engine/create-surveys-engine.ts'))).toBe(true)
    expect(files.some((f) => f.endsWith('lib/surveys-engine/frequency.ts'))).toBe(true)
    ...
    expect(files.some((f) => f.endsWith(join('src', 'core', 'scoring.ts')))).toBe(true)
    expect(files.length).toBeGreaterThanOrEqual(12)

    // §0 C4: `src/core/` is allowed, `src/types/` is not — that is the rule
    // that still discriminates once the allow-set widens.
    const ALLOWED = [
      join('lib', 'surveys-engine'),
      join('src', 'core'),
      // The ONE file under src/types/ the engine may reach. It has zero imports
      // of its own — verified by the `REACT_TYPES` rule below AND by the
      // per-file `react` / `@tour-kit/media` scans every file here gets.
      join('src', 'types', 'scoring.ts'),
    ]
    /**
     * The React-carrying type modules, NAMED rather than blanket-banning
     * `../types`. Measured 2026-09-10: `types/survey.ts` type-imports ReactNode,
     * `types/question.ts` reaches `@tour-kit/media`, and the bare `../types`
     * barrel re-exports both. `types/scoring.ts` has ZERO imports of its own and
     * is genuinely React-free, which is why `core/scoring.ts` and the engine
     * factory take their three result types from it. Banning it would be a false
     * red; exempting those two FILES instead would let a later `../types/survey`
     * import slip into them unseen.
     */
    const REACT_TYPES = /from ['"](\.\.\/)+types(\/(survey|question|context))?['"]/
    ...
    for (const file of files) {
      ...
      expect(
        REACT_TYPES.test(source),
        `${file} imports a React-carrying types module (survey / question / context)`
      ).toBe(false)
