/**
 * v3 Phase 2 — `@tour-kit/checklists/engine` ships no React.
 *
 * US-1: a Vue/Svelte/vanilla developer runs checklists with React not
 ...
 *
 * Copied from `packages/hints/src/__tests__/no-react-in-engine-dist.test.ts`.
 * Like hints (and unlike scheduling and analytics) this package builds with
 * `splitting: true`, so `dist/engine/index.js` is a re-export shell and the
 * code lives in `chunk-*.js` beside it. A scan of the shell passes forever, no
 * matter what the package imports. So every scan below reads `closureSrc(...)`
 * — the same walker the size gate uses — and one case asserts the closure is
 * bigger than the shell could possibly be.
 *
 * The checklists-specific rule is in the source walk: every file the barrel
 * reaches must live under `src/lib/checklists-engine/`. The package's own
 * `src/types/checklist.ts` imports `react` for `ReactNode` and
 * `@tour-kit/media` for `MediaSlotProps`, and one
 * `import type … from '../../types'` in a lib file would put both in the
 * declaration closure.
 */
...
/** Measured on the main closure, not assumed. Deliberately not FORBIDDEN. */
const CONTROL_JS = [
  'react',
  'react/jsx-runtime',
  '@tour-kit/media',
 ...
 * `@tour-kit/core/engine` by design, so a forbidden-list entry for bare core
 * would trip on every correct build.
 */
const BARE_CORE = new RegExp(`${SPECIFIER_PREFIX}["']@tour-kit/core["']`)

describe('@tour-kit/checklists/engine ships no React', () => {
  ...
  it.skipIf(!distExists())('ANTI-VACUITY — the engine closure is more than the shell', () => {
    // Two files is the NORMAL shape for a split package, so the file count
    // discriminates almost nothing. The BYTE count is the load-bearing half:
    // the shell alone is a few hundred bytes, a real closure many KB.
  ...
  it.skipIf(!distExists())('CONTROL — the main closure trips the same matchers', () => {
    ...
    // Positive control for BARE_CORE: this package's main closure genuinely
    // imports bare `@tour-kit/core`. Without this, a BARE_CORE that silently
    // stopped matching would turn the engine's "never bare core" case below
    // into a vacuous pass.
  ...
  it.skipIf(!distExists())('the engine imports @tour-kit/core/engine, never bare core', () => {
    // The one bare specifier the engine is allowed. Bare `@tour-kit/core`
    // would pull the main barrel's providers, fourteen hooks and `UnifiedSlot`
    // into a Vue consumer's graph.
  ...
  it.skipIf(!distExists())('core is externalised, not bundled in', () => {
    ...
    // be vacuous forever. `createTourEngine` is a core symbol checklists
  ...
  it.skipIf(!distExists())("'use client' lands on the React entries only", () => {
    // No `headless` entry in this package — the main pair is the whole of the
    // React side.
  ...
  it('every source file the engine barrel reaches lives under lib/checklists-engine/', () => {
    const files = reachableFrom(BARREL)
    // A silently-empty walk passes every assertion below it forever. Say out
    // loud what it must have found: the barrel plus ten lib modules is today's
    // exact count — a floor, not a target.
    expect(files.some((f) => f.endsWith('lib/checklists-engine/reducer.ts'))).toBe(true)
    expect(files.some((f) => f.endsWith('lib/checklists-engine/create-checklists-engine.ts'))).toBe(
      true
    )
    expect(files.some((f) => f.endsWith('lib/checklists-engine/url-visit-tasks.ts'))).toBe(true)
    expect(files.length).toBeGreaterThanOrEqual(11)

    for (const file of files) {
      const source = read(file)
      expect(file.endsWith('.tsx'), `${file} is a React file`).toBe(false)
      // The checklists-specific rule. `src/types/checklist.ts` imports react
      // and @tour-kit/media, so one `import type … from '../../types'` in a
      // lib file would put both in the declaration closure — and a TYPE import
      // leaves no trace in the JS, so only this case would catch it.
      if (file !== BARREL) {
        expect(
          file.includes(`${join('lib', 'checklists-engine')}`),
          `${file} is reachable from the engine barrel but is not under lib/checklists-engine/`
        ).toBe(true)
      }
  ...
  it('the engine entry is never stamped with a client directive', () => {
    ...
    // entry to `injectUseClient` is a one-word change that the built-bytes
    // case only catches after a rebuild.
