...
import type { EngineChecklistConfig, EngineTaskConfig } from './types'
 ...
 * ```
 */
export function createChecklist<TConfig extends EngineChecklistConfig>(config: TConfig): TConfig {
 ...
 */
export function createTask<TTask extends EngineTaskConfig>(config: TTask): TTask {
  return config
}
