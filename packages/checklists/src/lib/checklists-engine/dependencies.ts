 ...
 */
export function resolveTaskDependencies<TTask extends EngineTaskConfig>(tasks: TTask[]): TTask[] {
  const resolved: TTask[] = []
  const seen = new Set<string>()
  const visiting = new Set<string>()
  const path: string[] = []

  function visit(task: TTask) {
 ...
 */
export function hasCircularDependency(tasks: EngineTaskConfig[]): boolean {
